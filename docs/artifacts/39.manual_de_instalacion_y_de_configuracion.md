...
| **Artefacto anterior** | [38. Plan de Despliegue y Puesta en Operación](38.plan_de_despliegue_y_puesta_en_operacion.md) |
| **Artefacto siguiente** | [40. Manual de Operación del Producto](40.manual_de_operacion_del_producto.md) |
| **Referencias** | Artefacto 38 (Plan de Despliegue) · Artefacto 29 (Diagrama de Despliegue) · `DEPLOY.md` |
...
|---------|-------|-------|-------------|
| 1.0.0 | 2026-03-27 | GitHub Copilot — Agente PO Ceiba | Creación inicial. Basado en análisis directo de los archivos fuente del proyecto. Incluye 7 diagramas Mermaid y documentación de seguridad, beans, interceptores, guards, servicios HTTP, proxy, pruebas. |
| 2.0.0 | 2026-04-27 | GitHub Copilot — Agente Arquitecto Ceiba | **Actualización por Dockerización.** Sección 4.4 (prerrequisitos Docker). Sección 5 (estructura del proyecto con Dockerfiles, nginx.conf, .env.example, DEPLOY.md). Sección 6.2 (application.properties actualizado con patrón `${VAR:default}` de externalización). Sección 7.5 (CORS ahora parametrizable vía `app.cors.allowed-origins`, ya no usa constante `LOCAL_FRONT_URL` hardcodeada). Sección 10.3 (`environment.prod.ts` ya existe y `angular.json` ya tiene `fileReplacements` configurado). Corrección de número de artefacto (era 38, correcto es 39). |
