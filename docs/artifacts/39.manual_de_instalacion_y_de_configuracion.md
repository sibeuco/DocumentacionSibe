# Artefacto 38 — Manual de Instalación y de Configuración
...
| **Artefacto anterior** | [37. Plan de Despliegue y Puesta en Operación](37.plan_de_despliegue_y_puesta_en_operacion.md) |
| **Artefacto siguiente** | [39. Manual de Usuario](39.manual_de_usuario.md) |
| **Referencias** | Artefacto 37 (Plan de Despliegue) · Artefacto 36 (Certificación Funcional) |
...
- [Artefacto 38 — Manual de Instalación y de Configuración](#artefacto-38--manual-de-instalación-y-de-configuración)
  ...
  - [2. Relación con el Artefacto 37](#2-relación-con-el-artefacto-37)
...
A diferencia del Artefacto 37 (Plan de Despliegue), que define el *qué* y el *cuándo* del despliegue, este manual documenta el *cómo* de cada configuración, explicando las decisiones técnicas implementadas en el código fuente.

---

## 2. Relación con el Artefacto 37
...
| Aspecto | Artefacto 37 | Artefacto 38 |
|---------|-------------|-------------|
...
| **Contenido** | Comandos de despliegue, rollback, checklis | Estructura de archivos, parámetros, diagramas, explicación de clases |
...
C4Context
    ...
    System(sibe, "SIBE", "Sistema de Información de Bienestar Estudiantil\n(Angular SPA + Spring Boot REST API)")
...
**Contenido actual (entorno de desarrollo):**
```properties
...
# NOTA: Las credenciales no deben estar en texto claro en producción.
# Usar variables de entorno (ver §5.1 del Artefacto 37).

# ─── JPA / Hibernate ───────────────────────────────────────────────
spring.jpa.properties.hibernate.dialect=org.hibernate.dialect.PostgreSQLDialect
spring.jpa.open-in-view=false
spring.jpa.hibernate.ddl-auto=update
# 'update': Hibernate actualiza el esquema al iniciar (solo para desarrollo).
# En producción cambiar a 'validate' (ver §15.4 del Artefacto 37).
...
spring.mail.username=sibeapplicationuco@gmail.com
spring.mail.password=atsiysuskgnqccrk       # ← App Password de Gmail
...
| `spring.jpa.hibernate.ddl-auto` | `update` | Hibernate crea/actualiza tablas automáticamente. **Solo desarrollo.** |
...
| `exp` | Expiration | Date | Fecha de expiración (iat + 30 000 000 ms ≈ 8.3 horas) |
...
stateDiagram-v2
    ...
    Válido --> Expirado: Tras ~8.3 horas\nde inactividad
```

**Tiempo de expiración:**
```java
// NumeroConstante.java
public static final long TREINTA_MILLONES = 30_000_000L;   // milisegundos
// 30.000.000 ms = 30.000 s = 500 min ≈ 8.33 horas
```

### 7.5 Configuración CORS

```java
// ProjectSecurityConfig.java — configuración CORS
config.setAllowedOrigins(List.of(LOCAL_FRONT_URL));     // "http://localhost:4200"
...
|--------------|-------------|----------------------|
| `allowedOrigins` | `http://localhost:4200` | **CRÍTICO:** Debe cambiarse al dominio real (e.g. `https://sibe.uco.edu.co`) |
...
**Archivo a crear — `src/environments/environment.prod.ts` (producción):**  
> ⚠️ Este archivo **NO existe actualmente** en el proyecto y debe crearse antes del despliegue en producción.

```typescript
export const environment = {
  production: true,
  endpoint: '/api'
  // Rutas relativas: nginx en el servidor de producción hace proxy_pass
  // de /api/* → Spring Boot :8080/api/*
  // Alternativa con dominio absoluto:
  // endpoint: 'https://sibe.uco.edu.co/api'
};
```

**Configuración de `fileReplacements` en `angular.json`** (para que el build de producción use `environment.prod.ts`):
```json
"configurations": {
  "production": {
    "fileReplacements": [
...
Este interceptor actúa **before** de AuthInterceptor (se registra primero en CoreModule). Opera con el token almacenado en **cookies** (via `ngx-cookie-service`):

```typescript
// TokenInterceptor — lógica de validación
const token = this.cookieService.get('token');   // Lee token de cookie

if (token && this.isTokenExpired(token)) {
    this.logout();                    // Limpia sessionStorage + localStorage + cookie
...
> ⚠️ **Observación de arquitectura:** El proyecto usa **dos mecanismos de almacenamiento** para el JWT:
> - `AuthInterceptor` usa `sessionStorage['Authorization']` (string directo del header).
> - `TokenInterceptor` usa `CookieService.get('token')` (cookie).
>
> `securityGuard` usa `sessionStorage.getItem('Authorization')`.
>
> Esto puede causar inconsistencia en el ciclo de vida de la sesión. Para producción, se recomienda unificar el mecanismo de almacenamiento.

**Método `logout`:**
```typescript
private logout(): void {
    this.cookieService.delete('token');  // Elimina cookie
...
sequenceDiagram
    ...
    alt Token encontrado y no expirado
        TI->>TI: req.clone({headers: 'Authorization: Bearer <token>'})
        TI->>AI: next.handle(authReq)
    else Sin token en cookie
    ...
    alt Hay userdetails (login)
        ...
        AI->>AI: Bearer token del header anterior
    end
...
> **Estado actual de cobertura (Artefacto 36):** 1.89% de cobertura en el backend. Los tests de la capa de dominio son unitarios reales; los de infraestructura usan `@Mock` sin wiring real, por lo que no generan cobertura de instrucción significativa.
...
|---------|-------|-------|-------------|
| 1.0.0 | 2026-03-27 | GitHub Copilot — Agente PO Ceiba | Creación inicial. Basado en análisis directo de: `ProjectSecurityConfig.java`, `UsernamePwdAuthenticationProvider.java`, `SeguridadConstante.java`, `ApiEndpointConstante.java`, `JWTTokenGeneratorFilter.java`, `JWTTokenValidatorFilter.java`, `ExceptionFilter.java`, `RequestValidationBeforeFilter.java`, `ManejadorError.java`, `EncriptarClaveBean.java`, `ServiceBean.java`, `app.module.ts`, `core.module.ts`, `shared.module.ts`, `app-routing.module.ts`, `auth-interceptor.ts`, `token-interceptor.ts`, `manejador-error.ts`, `http-codigo-error.ts`, `security.guard.ts`, `public-route.guard.ts`, `http.service.ts`, `environment.ts`, `proxy.conf.json`, `build.gradle`, `application.properties`. Incluye 7 diagramas Mermaid: dependencias entre capas, flujo de autenticación, cadena de filtros, ciclo de vida JWT, módulos frontend, flujo de interceptores, navegación con guards. |
