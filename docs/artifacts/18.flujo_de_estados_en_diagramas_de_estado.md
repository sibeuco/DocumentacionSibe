...
Este artefacto documenta los **diagramas de estado** (UML State Machine Diagrams) de todas las entidades del sistema SIBE (Sistema de Información de Bienestar Estudiantil) que poseen un **ciclo de vida con estados discretos**. A diferencia del artefacto 17 que traza flujos de transacciones (actividades), este artefacto modela el **comportamiento reactivo** de cada entidad a lo largo del tiempo: cómo responde a eventos externos y bajo qué condiciones transiciona entre estados.
...
Los diagramas se derivan directamente del **código fuente** del backend (Java/Spring Boot, arquitectura hexagonal) y del frontend (Angular), trazando cada transición hasta los casos de uso, servicios y repositorios que la implementan.
...
### 3.6 Hallazgo: Cancelación No Reinicia horaInicio

Al ejecutar `CancelarActividadUseCase`, el estado retorna a `PENDIENTE` pero el campo `horaInicio` **conserva el valor** de cuando se inició la actividad. Esto significa que si la actividad se reinicia posteriormente, el campo `horaInicio` se sobreescribirá con el nuevo `LocalTime.now()`, pero entre la cancelación y el nuevo inicio, la ejecución en estado PENDIENTE porta una `horaInicio` residual del intento anterior.
...
### 4.6 Hallazgo: Ausencia de Reactivación

No existe en el código actual un mecanismo para reactivar un usuario desactivado. El estado INACTIVO es **terminal e irreversible** en la implementación actual. Si se requiere rehabilitar un usuario previamente eliminado, sería necesario una intervención directa en la base de datos (`UPDATE usuario SET esta_activo = true WHERE ...`).
...
### 5.6 Hallazgo: Falta de Limpieza de Peticiones Expiradas

No existe un proceso batch, cron job, ni scheduled task que elimine filas expiradas de la tabla `peticion_recuperacion_clave`. Las filas expiradas se acumulan indefinidamente en la BD a menos que el usuario vuelva a solicitar un código (lo cual sobreescribe la fila existente). Esto podría representar una acumulación no controlada de datos.
...
### 6.6 Hallazgo: JWT Irrevocable

El sistema no implementa un mecanismo de revocación de tokens (blacklist, token versioning, ni refresh tokens). Si un usuario es desactivado mientras tiene un JWT válido, ese token seguirá siendo aceptado por `JWTTokenValidatorFilter` hasta su expiración natural (~8.33 horas). Sin embargo, las operaciones que consultan datos del usuario via `PersonaRepositorioConsulta` fallarán porque retornarán `null` para usuarios inactivos.
...
Las siguientes entidades fueron evaluadas durante la investigación y se confirmó que **no poseen campos de estado ni transiciones entre valores discretos**. Se incluyen para completitud y trazabilidad de la evaluación.
...
| # | Hallazgo | Entidad | Severidad | Descripción |
|---|----------|---------|-----------|-------------|
| 1 | **Cancelación no reinicia `horaInicio`** | EjecucionActividad | Baja | Al cancelar (EN_CURSO → PENDIENTE), el campo `horaInicio` conserva el valor del inicio anterior. El dato residual persiste hasta el siguiente inicio. |
| 2 | **Ausencia de reactivación de usuario** | Usuario | Media | No existe `ReactivarUsuarioUseCase`. El estado INACTIVO es terminal e irreversible sin intervención directa en BD. |
| 3 | **Acumulación de peticiones OTP expiradas** | PeticionRecuperacionClave | Baja | No hay proceso batch para eliminar filas expiradas. Se acumulan indefinidamente a menos que el usuario re-solicite. |
| 4 | **JWT irrevocable** | Sesión JWT | Media | No hay blacklist ni versionado de tokens. Un JWT emitido sigue siendo válido hasta su expiración natural (~8.33h), incluso si el usuario es desactivado. |
| 5 | **Estado CANCELADA inexistente** | EjecucionActividad | Baja | La cancelación revierte a PENDIENTE en lugar de transicionar a un estado CANCELADA dedicado, lo que dificulta la auditoría de cancelaciones. |
| 6 | **Ventana OTP sin configuración externa** | PeticionRecuperacionClave | Baja | Los 5 minutos de ventana de validez están hardcoded en el caso de uso. No son configurables via propiedades externas. |
