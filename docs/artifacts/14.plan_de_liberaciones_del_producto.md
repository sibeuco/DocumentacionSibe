...
- La inspección directa del código fuente para verificar el estado real de implementación.
...
| ¿Qué riesgos enfrenta cada release? | Matriz de riesgos con probabilidad, impacto y mitigación. |
| ¿Cuál es el estado actual de implementación? | Verificación contra el código fuente real. |
...
| 5 | **Incremento decreciente** | Cada release sucesivo es más pequeño (7 → 6 → 2 historias), reflejando que el grueso del valor se entrega tempranamente. |
...
│   (Walking Skeleton)│   (Cap. Completa)   │   (Robustez + Estrategia)           │
│                     │                     │                                      │
│   7 HU · 5/5 back. │   6 HU · 5/5 back. │   2 HU · 2/5 back.                  │
│   8 comandos        │   14 comandos       │   3 comandos                         │
│   29 escenarios     │   23 escenarios     │   9 escenarios                       │
│                     │                     │                                      │
│   Flujo completo    │   Operación         │   Seguridad proactiva               │
│   login → dashboard │   autónoma total    │   + indicadores estratégicos         │
└─────────────────────┴─────────────────────┴──────────────────────────────────────┘
         ▲                     ▲                          ▲
         │                     │                          │
...
| Cobertura del Backbone | 5/5 (100 %) | 5/5 (100 %) | 2/5 (40 %) | 5/5 |
| Comandos de escritura | 8 | 14 | 3 | **25** |
| Escenarios de aceptación | 29 | 23 | 9 | **61** |
| Complejidad dominante | Media-Alta | Media-Alta | Media-Baja | — |
...
graph LR
    ...
    WS3["HU-008<br/>Cargar Estudiantes"]
    WS4["HU-010<br/>Crear Actividad"]
    WS5["HU-012<br/>Registrar Asistencia"]
    WS6["HU-015<br/>Ver Dashboard"]
...
**Flujo narrativo:** El Administrador inicia sesión → consulta la estructura organizacional → carga el archivo Excel de estudiantes → crea una actividad con fechas → el Colaborador inicia la actividad y registra asistencia RFID → el Administrador visualiza métricas de participación en el dashboard.
...
```
HU-001 (R1) → HU-005 (R1) → HU-006 (R2) → HU-007 (R3)
```

Esta ruta conecta la autenticación base con la capa completa de configuración estratégica, pasando por estructura organizacional y proyectos antes de llegar a indicadores.
...
**Objetivo:** Demostrar, en un único ciclo de entrega, que la cadena de valor completa del sistema SIBE funciona de principio a fin: desde que un usuario se autentica hasta que un administrador visualiza métricas de participación. Este release no persigue amplitud funcional, sino profundidad vertical: cada capa (autenticación, configuración, carga de datos, planificación, ejecución y analítica) debe operar de forma articulada y producir valor observable para la Dirección de Bienestar y Evangelización.

La metáfora del *Walking Skeleton* es deliberada: el sistema debe ser capaz de "caminar" —ejecutar un flujo de negocio completo— aunque todavía no pueda "correr". Las siete historias de usuario que componen este release son el conjunto mínimo e irreducible para que ese flujo exista: retirar cualquiera de ellas rompe la cadena.
...
| HU-010 | Creación y Edición de Actividades y su Programación de Fechas | A4 — Actividades | Permite crear actividades con datos maestros (nombre, objetivo, semestre, indicador, área, colaborador) y programar N fechas de ejecución, generando automáticamente N instancias de `EjecucionActividad` en estado `PENDIENTE`. Soporta edición de actividades existentes y adición de nuevas fechas. | ADMINISTRADOR_DIRECCION, ADMINISTRADOR_AREA | 4 |
...
|-------|-----------|---------------|
| 🔴 Alta | HU-008 | 14 % |
| 🟡 Media | HU-002, HU-010, HU-012 | 43 % |
| 🟢 Baja | HU-001, HU-005, HU-015 | 43 % |

> **Valoración global:** Complejidad **Media-Alta**. El Release 1 contiene un único componente de riesgo transaccional elevado (HU-008), pero el grueso de las historias opera a complejidad media o baja. El mayor desafío de integración no es transaccional sino de hardware: la dependencia del lector RFID en HU-012.
...
graph TD
    ...
    HU005 -->|"área para nueva actividad"| HU010
...
HU-001 es el nodo raíz del grafo: toda interacción con el sistema requiere sesión autenticada. Desde HU-001 se bifurcan tres ramas iniciales:
...
- **Rama de participantes:** HU-001 → HU-008. La carga masiva de estudiantes es independiente de la estructura (las Áreas son del personal, no de los estudiantes) y puede ejecutarse en paralelo con HU-002.
- **Rama core:** HU-005 → HU-010 → HU-012 → HU-015. Las Áreas habilitan la creación de actividades; las actividades generan ejecuciones; las ejecuciones son el contexto en que se registra asistencia; la asistencia registrada alimenta el dashboard.

El punto de convergencia crítico es HU-012 que depende tanto de HU-010 (que exista una ejecución) como de HU-008 (que existan participantes en la base de datos). Ambas ramas deben completarse antes de poder validar el registro de asistencia.
...
```
HU-001 → HU-005 → HU-010 → HU-012 → HU-015
```
...
| 2 | HU-005 | Sus endpoints son requeridos por HU-010 para el selector de Área y por HU-002 para la asignación de usuarios; debe estar operativo antes que ambos. |
| 3 | HU-010 | Crea las `EjecucionActividad` en estado `PENDIENTE` que HU-012 necesita para poder iniciar una actividad. |
| 4 | HU-012 | El core del sistema; solo puede validarse después de HU-010 (ejecución existe) y HU-008 (participantes en BD). |
| 5 | HU-015 | El dashboard solo muestra datos reales después de que HU-012 haya registrado al menos una asistencia. |

**Longitud de la ruta:** 4 aristas — la secuencia más larga posible dentro del Release 1.

**Por qué esta es la ruta crítica:** Cualquier bloqueo en uno de estos nodos congela todo el trabajo posterior en la cadena. Específicamente, HU-012 actúa como cuello de botella porque es la convergencia de dos ramas: si HU-010 o HU-008 se retrasan, HU-012 no puede completarse, y en consecuencia HU-015 tampoco.
...
| # | Criterio | Verificable por |
|---|----------|-----------------|
| CS-01 | Las 7 historias de usuario de R1 están implementadas en backend y frontend y fusionadas en la rama principal. | Revisión de código / CI pipeline verde |
| CS-02 | La cobertura de pruebas unitarias e integración del backend es ≥ 90 %. | Reporte JaCoCo |
| CS-03 | Todos los escenarios de aceptación de las 7 HU han sido validados y aprobados por el Product Owner. | Acta de revisión de sprint |
| CS-04 | El flujo completo Walking Skeleton es ejecutable de extremo a extremo: Login → Estructura → Carga → Actividad → Asistencia RFID → Dashboard. | Demo en entorno de staging |
...
|----|--------|:------------:|:-------:|--------------------------|
| R1-01 | **Dependencia de hardware RFID:** El lector RFID físico no está disponible durante el desarrollo o presenta incompatibilidades con el protocolo de lectura de carnets universitarios, bloqueando la validación de HU-012. | Alta | Alto | Diseñar la interfaz `AttendanceRecordComponent` con una capa de abstracción que permita sustituir el lector físico por un campo de entrada manual. Coordinar temprano la entrega del hardware con la oficina de sistemas. |
| R1-02 | **Cambio de esquema del archivo Excel de estudiantes:** El área de sistemas universitaria modifica las columnas, nombres o tipos de datos del `.xlsx` semestral, rompiendo el `ProcesadorArchivoEstudianteServicio` de HU-008. | Media | Alto | Implementar validación de esquema al inicio del procesamiento (fail-fast). Parametrizar los nombres de columna en un archivo de configuración. Obtener una muestra real del archivo antes de finalizar HU-008. |
| R1-03 | **Degradación de rendimiento en carga masiva:** Un archivo con más de 5.000 estudiantes excede el tiempo aceptable de procesamiento (< 2 s según RNF-002) o genera un `OutOfMemoryError`. | Media | Medio | Implementar lectura en streaming del Excel (Apache POI `SAX` mode) en lugar de carga completa en memoria. Considerar procesamiento por lotes con transacciones parciales. |
| R1-04 | **Dependencia circular en UI entre HU-002 y HU-005:** El formulario de creación de usuarios (HU-002) requiere la lista de Áreas de HU-005 para poblar el desplegable. Si HU-005 no está lista, HU-002 no puede completarse. | Alta | Medio | Definir HU-005 como precondición explícita de HU-002. Priorizar la implementación del endpoint `GET /areas` de HU-005 en el primer sprint, antes de iniciar el frontend de HU-002. |
| R1-05 | **Seguridad del token JWT:** Almacenamiento inseguro del JWT en `localStorage` (vulnerable a XSS) o configuración débil de expiración/algoritmo. | Media | Alto | Evaluar uso de cookies `HttpOnly` + `Secure`. Configurar expiración corta (ej. 8 h). Usar algoritmo HS256 con secreto de longitud adecuada (≥ 256 bits). |
| R1-06 | **Concurrencia en el registro de asistencia:** Dos escaneos del mismo carnet en ráfaga rápida (< 200 ms) pueden eludir la detección de duplicados si la lógica no usa un índice único a nivel de BD. | Baja | Medio | Añadir restricción `UNIQUE (ejecucion_id, participante_id)` en la tabla `Asistencia`. Manejar la `DataIntegrityViolationException` y devolver respuesta de "ya registrado". |

---

### 5.8 Trazabilidad a Requerimientos
...
> **Release 1 — Fundación está completamente implementado.** Las 7 historias de usuario han concluido en backend (arquitectura hexagonal + CQRS), frontend (Angular) y pruebas automatizadas, con una cobertura de backend del **96 %**. El flujo completo Walking Skeleton `Login → Estructura Organizacional → Carga de Estudiantes → Creación de Actividad → Registro de Asistencia RFID → Dashboard de Métricas` es operativo y validado.
...
**Objetivo:** Completar las capacidades operativas del sistema SIBE sobre la base funcional establecida por Release 1. Esta entrega incorpora la recuperación autónoma de acceso, la gestión del plan estratégico, la incorporación de empleados como participantes, la consulta contextualizada de actividades por rol y contexto organizacional, el cierre formal del ciclo de vida de las actividades y la generación de reportes de asistencia descargables en formato Excel.

Con este release, el equipo de bienestar puede operar SIBE de forma completamente autónoma: sin depender del administrador para tareas de recuperación de acceso, con el catálogo completo de participantes (estudiantes + empleados), con el cierre formal de actividades sellando la trazabilidad de asistencia, y con evidencia exportable para indicadores institucionales.
...
| HU-006 | Gestión de Proyectos del Plan de Desarrollo | A2: Config. | CRUD completo de Acciones (entidades base sin cascadas) y Proyectos (vinculados a N acciones vía tabla de unión `ProyectoAccion`). Marco estratégico del que dependen indicadores y actividades. | ADMINISTRADOR_DIRECCION | 5 |
| HU-009 | Carga Masiva de Empleados (Excel) | A3: Participantes | Upload de `.xlsx` con upsert transaccional por número de documento. Tres entidades en cascada por fila: `Empleado`, `EmpleadoRelacionLaboral`, `EmpleadoCentroCostos`. Validación mediante cuatro motores de dominio. Mayor complejidad que HU-008 (estudiantes). | ADMINISTRADOR_DIRECCION | 3 |
| HU-011 | Consulta de Actividades por Contexto Organizacional | A4: Actividades | `ADMINISTRADOR_DIRECCION` visualiza todas (endpoint `por-direccion`). `ADMINISTRADOR_AREA` ve solo su área (`por-area`). `COLABORADOR` ve solo su subárea (`por-subarea`). Drill-down a fechas de ejecución. Control mediante `AutorizacionContextoOrganizacionalServicio`. | Todos los roles | 4 |
...
**Total: 6 historias · 23 escenarios de aceptación · Cobertura backbone 5/5**
...
**Observación clave:** No existen dependencias internas *hard* entre las 6 historias de R2. Todas dependen exclusivamente de historias de R1 (ya completamente implementadas). Esto significa que las 6 historias de R2 pueden desarrollarse y verificarse en paralelo, sin bloqueos entre sí.
...
|:---------:|----------|:------:|---------------|
| 1 | HU-009 | 🔴 | Carga masiva con cascadas triples por fila; mayor riesgo transaccional del release |
| 2 | HU-013 | 🔴 | N inserts atómicos en estado terminal irreversible |
| 3 | HU-003 | 🔴 | 3 comandos encadenados + SMTP externo asíncrono |
| 4 | HU-006 | 🟡 | Diff sobre tabla de unión ProyectoAccion |
| 5 | HU-011 | 🟡 | Autorización contextual por contexto organizacional |
| 6 | HU-014 | 🟢 | Solo lectura, generación client-side |

**Camino de mayor valor de negocio acumulado:** HU-009 → HU-013 → HU-011 → HU-014 (completa participantes → cierra actividades → consulta contextual → evidencia exportable).
...
| E4 | Archivo Excel de empleados en formato válido disponible (para HU-009) | Coordinar con sistemas |
| E5 | Al menos una ejecución de actividad en estado `EN_CURSO` con asistencias (para HU-013 y HU-014) | Datos de prueba R1 |

#### Criterios de Salida

| # | Criterio |
|---|----------|
| S1 | Las 6 historias han superado la totalidad de sus 23 escenarios de aceptación. |
| S2 | Los 10 comandos de R2 tienen cobertura de pruebas unitarias e integración en el backend. |
...
| S4 | La carga de HU-009 con archivo de muestra representativo (mínimo 200 filas) ejecuta en < 2 s, cumpliendo RNF-002. |
| S5 | Las consultas de HU-011 verifican correctamente el contexto organizacional: ningún rol obtiene actividades fuera de su ámbito. |
| S6 | La finalización de HU-013 es atómica: si cualquier insert falla, ningún registro parcial persiste. |
| S7 | El reporte de HU-014 genera correctamente un `.xlsx` descargable con estructura Dirección → Área → Actividad → Participantes. |
| S8 | La suite de pruebas de Release 1 pasa al 100 % sin regresiones. |
...
| ID | Riesgo | Probabilidad | Impacto | Estrategia de Mitigación |
|----|--------|:------------:|:-------:|--------------------------|
| R2-01 | **Estado inconsistente por fallo SMTP post-persistencia** (HU-003): El código se guarda en BD pero el correo no se entrega. El usuario no puede recuperar acceso. | Media | Alto | Implementar reintento con `@Retryable` sobre el `@Async`. Proveer endpoint auxiliar para reenviar el código sin regenerar uno nuevo. |
| R2-02 | **Enumeración de usuarios** (HU-003): Si la respuesta distingue entre "correo no registrado" y "código incorrecto", un atacante mapea usuarios del sistema. | Media | Alto | Responder con mensaje genérico idéntico sin importar si el correo existe. Verificar explícitamente en pruebas de seguridad. |
| R2-03 | **Timeout HTTP en carga de empleados** (HU-009): Los N × 3 upserts en cascada para 500+ empleados pueden superar el timeout del contenedor. | Media | Alto | Configurar `server.connection-timeout` y `spring.mvc.async.request-timeout`. Evaluar procesamiento asíncrono con `jobId` + polling para cargas grandes. |
| R2-04 | **Fallo de integridad en finalización masiva** (HU-013): Un `ConstraintViolationException` en el insert _k_ de N provoca rollback total. | Media | Alto | Pre-validar la lista completa de participantes antes de iniciar los inserts. Diseñar el comando como idempotente. |
| R2-05 | **Bypass de autorización contextual** (HU-011): Un `ADMINISTRADOR_AREA` invoca directamente `GET /actividades/area/{id_ajena}`. | Media | Alto | `AutorizacionContextoOrganizacionalServicio` debe validar contra el JWT del `SecurityContext`. Cubrir con prueba de seguridad negativa. |
| R2-06 | **Desbordamiento de memoria en el navegador** (HU-014): `ExcelReportService` construye el workbook completo en memoria para datasets grandes. | Baja | Medio | Implementar paginación en endpoints `/detalle` o límite configurable de filas. Alternativa a largo plazo: streaming desde servidor. |

---

### 6.8 Trazabilidad a Requerimientos
...
### 6.9 Estado de Implementación

| Código | Historia | Backend | Frontend | Pruebas | Estado |
|--------|----------|:-------:|:--------:|:-------:|:------:|
| HU-003 | Recuperación de Contraseña (Olvido) | ✅ | ✅ | ✅ | ✅ Terminado |
| HU-006 | Gestión de Proyectos del Plan de Desarrollo | ✅ | ✅ | ✅ | ✅ Terminado |
| HU-009 | Carga Masiva de Empleados (Excel) | ✅ | ✅ | ✅ | ✅ Terminado |
...
> **Release 2 — Operativo está completamente implementado.** Las 6 historias de usuario están en estado `Completado`. SIBE alcanza su capacidad operativa plena: catálogo completo de participantes, ciclo de vida de actividades con cierre formal, consulta contextual por rol, y evidencia exportable en Excel. **Nota:** HU-014 genera el reporte completamente en el navegador (`ExcelReportService` + librería `xlsx`), sin endpoint dedicado de backend; reutiliza las consultas `/detalle` de R1.
...
**Tema:** Consolidación de madurez — robustez de seguridad operativa e indicadores estratégicos.

**Objetivo:** Consolidar la madurez funcional de SIBE en dos frentes de alto valor no transaccional: la **robustez de seguridad operativa** sobre credenciales de usuarios autenticados y la **formalización de la capa estratégica** mediante indicadores vinculados al plan de desarrollo institucional.

El Release 3 no amplía el backbone completo del producto como lo hicieron los Releases 1 y 2, sino que profundiza capacidades ya existentes para cerrar brechas de madurez:

- Cierra el ciclo completo de gestión de credenciales con un mecanismo de cambio proactivo de contraseña.
- Completa la capa de parametrización estratégica que permite medir actividades contra proyectos.
- Habilita una relación más explícita entre operación y estrategia, sin alterar el núcleo transaccional ya entregado.

**Historias incluidas:** 2 · **Cobertura del backbone:** 2/5 actividades (A1 Acceso, A2 Configuración)
...
| HU-007 | Gestión de Indicadores Estratégicos | A2: Config. | Permite crear, consultar y modificar indicadores estratégicos asociados a proyectos del plan de desarrollo. Cada indicador tiene: nombre, tipo, temporalidad, proyecto vinculado y N públicos de interés. Admin Dirección tiene CRUD (sin eliminación); Admin Área tiene solo lectura. | ADMINISTRADOR_DIRECCION (CRUD), ADMINISTRADOR_AREA (lectura) | 5 |

**Total: 2 historias · 9 escenarios de aceptación**
...
#### Alcance específico de HU-007

- Creación de indicadores con cascadas JPA: 4 × `@OneToOne` (TipoIndicador, Temporalidad, Proyecto, Indicador) + N × `@OneToMany` (PublicoInteres).
- Modificación de indicadores existentes con actualización de todas las relaciones en cascada.
- Consulta de indicadores por ambos perfiles con permisos diferenciados.
- **Exclusión explícita de eliminación** por decisión de diseño institucional.
- **Dependencia sobre HU-010:** En R1, una actividad podía crearse sin indicador. En R3, los indicadores quedan disponibles para enriquecer la trazabilidad estratégica de nuevas actividades (dependencia opcional).
...
graph LR
    ...
    HU007 -.->|"asociación opcional\ndisponible en R3"| HU010

    style HU001 fill:#B0C4DE,color:#222
    style HU005 fill:#B0C4DE,color:#222
    style HU006 fill:#B0C4DE,color:#222
    style HU010 fill:#B0C4DE,color:#222
    style HU004 fill:#4A90D9,color:#fff
    style HU007 fill:#7B68EE,color:#fff
```

**Interpretación:**
- HU-004 depende únicamente de HU-001 (sesión activa). Puede desarrollarse de forma completamente independiente.
- HU-007 depende de HU-006 (R2), que a su vez depende de HU-005 (R1). Esta es la cadena cross-release más larga del producto: HU-001 → HU-005 → HU-006 → HU-007.
- Las dos historias de R3 no tienen dependencias entre sí; pueden desarrollarse en paralelo.
...
| # | Criterio |
|---|----------|
| S1 | Los 9 escenarios de aceptación (4 de HU-004 + 5 de HU-007) aprobados por el Product Owner. |
| S2 | `PUT /usuarios/modificar/clave` operativo con validación de contraseña actual, políticas y no-reutilización. |
| S3 | `POST/PUT/GET /indicadores` operativos con cascadas completas y permisos diferenciados. |
| S4 | Restricción de solo lectura para Administrador de Área verificada. |
| S5 | Ausencia de opción de eliminación de indicadores validada en frontend y backend. |
| S6 | Suite de pruebas de R1 y R2 sin regresiones. |
...
| ID | Riesgo | Probabilidad | Impacto | Estrategia de Mitigación |
|----|--------|:------------:|:-------:|--------------------------|
| R3-01 | **Inconsistencia de políticas de contraseña entre frontend y backend** (HU-004): Las reglas de validación no coinciden, generando aceptaciones visuales rechazadas por la API. | Media | Alto | Centralizar reglas de validación. Implementar pruebas de contrato y casos negativos compartidos entre capas. |
| R3-02 | **Bloqueo involuntario de acceso** (HU-004): Error en validación de contraseña actual, hash o persistencia deja al usuario sin acceso. | Baja | Alto | Probar flujos exitosos y fallidos exhaustivamente. Asegurar atomicidad del cambio. HU-003 (recuperación) actúa como contingencia. |
| R3-03 | **Inconsistencia en cascadas de indicadores** (HU-007): Errores de actualización duplican, omiten o dejan estados huérfanos en las relaciones JPA compuestas. | Media | Alto | Validaciones de integridad explícitas. Pruebas de actualización completa. Revisión del mapeo JPA cascade ALL. |
| R3-04 | **Exposición accidental de permisos de escritura a Admin Área** (HU-007): Control solo en frontend permite acceso vía API. | Media | Medio-Alto | Forzar control RBAC en `@PreAuthorize` del backend. Cubrir con prueba de seguridad negativa. |
| R3-05 | **Dependencia de calidad del catálogo de proyectos** (HU-007): Proyecto modificado o inconsistente afecta indicadores vinculados. | Baja | Medio | Validar existencia y vigencia del proyecto al guardar/modificar indicador. |

---

### 7.7 Trazabilidad a Requerimientos
...
- HU-007 fortalece RNF-006: institucionaliza un modelo estratégico formalizable, consultable y mantenible.
...
| HU-007 | Gestión de Indicadores Estratégicos | ✅ | ✅ | ✅ | ✅ Terminado |

> **Release 3 — Maduración está completamente implementado.** Las 2 historias del release están en estado `Completado`. Con esta entrega, el producto alcanza **15/15 historias completadas**, con una cobertura de pruebas backend del **96 %**. Release 3 actúa como release de cierre de madurez, completando la gestión de credenciales y la capa estratégica de indicadores.
...
**Interpretación estratégica:** Esta ruta conecta la autenticación base con la capa completa de configuración estratégica. La capacidad de medir impacto institucional (indicadores) solo se alcanza tras haber establecido acceso controlado (R1), estructura organizacional consultable (R1) y proyectos del plan de desarrollo (R2).

### 8.2 Evolución de Capacidades por Release

| Release | Tema | Qué agrega al producto | Backbone | Comandos | Escenarios |
|---------|------|------------------------|:--------:|:--------:|:----------:|
| R1 Fundación | Walking Skeleton | Login, gestión usuarios, estructura organizacional, carga de estudiantes, creación de actividades, registro asistencia RFID, dashboard base | 5/5 | 8 | 29 |
| R2 Operativo | Capacidad Completa | Recuperación contraseña, proyectos del plan, carga empleados, consulta contextual, cierre de actividades, reportes Excel | 5/5 | 14 | 23 |
| R3 Maduración | Robustez + Estrategia | Cambio proactivo de contraseña, indicadores estratégicos asociados a proyectos | 2/5 | 3 | 9 |
| **Total** | — | **Sistema completo** | **5/5** | **25** | **61** |

**Lectura evolutiva:**
- **R1** entrega valor de extremo a extremo y demuestra viabilidad del producto.
- **R2** completa la operación cotidiana y la evidencia exportable para indicadores institucionales.
- **R3** refina seguridad, gobierno y alineación estratégica sin expandir el flujo core.
...
|---------|------------|----------------|
| R1 | RNF-001, RNF-002, RNF-003, RNF-004, RNF-005 | 5 RNF |
| R2 | RNF-006 | 6 RNF |
| R3 | Fortalece RNF-004 y RNF-006 | 6 RNF |

> **Observación:** El mayor volumen de cobertura funcional se alcanzó en R1 (14 de 15 RF). R2 y R3 completan y profundizan los requerimientos restantes.

### 8.4 Matriz de Riesgos Consolidada

Los riesgos más significativos del producto, ordenados por severidad:

| # | Riesgo | Severidad | Releases | Descripción |
|---|--------|:---------:|----------|-------------|
| 1 | Secretos y credenciales hardcodeadas en `application.properties` | Crítica | R1-R3 | Contraseña de PostgreSQL (`admin1234`) y app password de Gmail SMTP en texto plano. Riesgo de exposición en repositorio. |
| 2 | Ausencia de migraciones formales de base de datos (Flyway/Liquibase) | Alta | R1-R3 | Esquema gestionado por JPA `auto-DDL`. Sin trazabilidad de cambios, sin reproducibilidad, sin rollback de esquema. |
| 3 | Cobertura de frontend superficial (156 spec files tipo `should create`) | Alta | R1-R3 | Backend muy cubierto (96 %), pero frontend carece de pruebas conductuales que detecten regresiones en UI. |
| 4 | Ausencia de suite de integración y e2e | Alta | R1-R3 | Sin `@SpringBootTest` en backend ni Cypress/Playwright en frontend. Validación full-stack depende de pruebas manuales. |
| 5 | Reintento sin cabeceras de autorización en `UploadDatabaseService` | Media-Alta | R1-R2 | El servicio reintenta la solicitud sin headers de autenticación ante 401/403. Comportamiento inseguro. |
| 6 | Reportes HU-014 generados completamente en el cliente | Media | R2 | Para conjuntos grandes, el navegador asume toda la carga de transformación y generación del `.xlsx`. |
| 7 | Divergencia de validaciones entre frontend y backend | Media | R2-R3 | Visible en flujos con reglas estrictas como contraseñas o formularios complejos. |

### 8.5 Deuda Técnica Identificada

| # | Deuda Técnica | Categoría | Efecto | Prioridad de Resolución |
|---|---------------|-----------|--------|------------------------|
| 1 | Credenciales hardcodeadas en `application.properties` para PostgreSQL y SMTP Gmail | Seguridad | Exposición de secretos; dificultad para promover entre ambientes | 🔴 Crítica |
| 2 | Sin Flyway/Liquibase; esquema depende de auto-generación JPA | Datos / DevOps | Cambios de esquema no trazables; despliegues no reproducibles | 🔴 Alta |
| 3 | 156 archivos spec del frontend con pruebas tipo `should create` | Calidad | Baja capacidad para detectar regresiones conductuales en UI | 🟡 Media |
| 4 | Sin suite dedicada de integración/e2e | Calidad sistémica | Riesgo de fallos en interacción real Angular ↔ Spring Boot ↔ BD | 🟡 Media |
| 5 | `UploadDatabaseService` reintenta sin cabeceras de auth tras 401/403 | Seguridad | Comportamiento inseguro ante errores de autorización | 🔴 Alta |
| 6 | HU-014 genera Excel desde el cliente sin endpoint dedicado de backend | Escalabilidad | Riesgo de consumo elevado de memoria en navegador con datasets grandes | 🟡 Media |

**Lectura arquitectónica:** La arquitectura base del backend es sólida (Spring Boot + hexagonal + CQRS + 96 % cobertura). La deuda pendiente se concentra en **industrialización**: gestión segura de secretos, ciclo de vida de BD, pruebas de integración real y escalabilidad de artefactos pesados.
...
|-----|:------------:|:------------:|:-------------:|
| Cobertura de historias | 100 % (7/7) | 100 % (6/6) | 100 % (2/2) |
| Cobertura del backbone | 5/5 | 5/5 | 2/5 |
| Escenarios aprobados | 29/29 | 23/23 | 9/9 |
