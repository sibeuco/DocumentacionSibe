...
| **Artefacto anterior** | [34. Cobertura de Código](34.cobertura_de_codigo.md) |
| **Artefacto siguiente** | [36. Certificación Funcional](36.certificacion_funcional.md) |
...
| Frontend — Departamento / Gestión | ~15 spec files | ~40 casos |
...
| ❌ Faltante / sin cobertura | Alta cobertura en lógica, 0% en ejecución real* | <2% de spec files sin prueba | — |

> **Nota sobre el backend:** Los ~273 casos de prueba están correctamente escritos a nivel de código, pero el reporte JaCoCo indica que el código de producción no se ejecuta durante las pruebas (ver Artefacto 34). Esto se debe al uso de `@Mock` sobre la clase bajo prueba en lugar de `@InjectMocks`. Los casos de prueba documentados aquí reflejan lo que los tests *intentan* verificar; su efectividad real requiere corrección estructural.
...
**Patrón de prueba aplicado:** AAA (Arrange-Act-Assert) con `@Mock` sobre los repositorios y `@InjectMocks` sobre la fábrica bajo prueba. *(Nota: ver advertencia en §3.4 sobre efectividad de cobertura actual.)*
...
| TC-FE-COR-029 | debería permitir acceso a gestionar-indicadores cuando rol es ADMINISTRADOR_AREA | Token con `rol=ADMINISTRADOR_AREA`, URL `/gestionar-indicadores` | Retorna `true` | Seguridad |
...
|----|---------------------|-------------------|------|
| TC-FE-COR-038 | should allow access when no token exists | Retorna `true` | Seguridad |
| TC-FE-COR-039 | should allow access when token is expired | Con JWT expirado, retorna `true` | Seguridad |
| TC-FE-COR-040 | should clear expired token from sessionStorage | Con JWT expirado, borra `Authorization` de `sessionStorage` | Seguridad |
| TC-FE-COR-041 | should redirect to /home when token is valid | Token válido → retorna `false`; navega a `/home` | Navegación |
| TC-FE-COR-042 | should allow access when token is malformed | Token malformado → retorna `true` | Seguridad |
...
| ID | Descripción del test | Qué verifica | Tipo |
|----|---------------------|--------------|------|
| TC-FE-COR-043 | should add Basic Authorization header when userdetails exist in sessionStorage | GET sale con `Authorization='Basic <base64>'` | Seguridad |
| TC-FE-COR-044 | should use Authorization from sessionStorage when no userdetails | Usa `Authorization='Bearer saved-token'` guardado | Seguridad |
| TC-FE-COR-045 | should not add Authorization header when no credentials exist | Sin credenciales, petición no incluye `Authorization` | Seguridad |
| TC-FE-COR-046 | should always add X-Requested-With header | `X-Requested-With: XMLHttpRequest` siempre presente | HTTP |
| TC-FE-COR-047 | should add X-XSRF-TOKEN header when XSRF-TOKEN is in sessionStorage | Agrega `X-XSRF-TOKEN: xsrf-value` | Seguridad |
| TC-FE-COR-048 | should remove userdetails from sessionStorage after intercept | `sessionStorage.getItem('userdetails')` queda `null` | Estado |
| TC-FE-COR-049 | should save Authorization header from response to sessionStorage | Respuesta con `Authorization: Bearer new-token` → guardado en `sessionStorage` | Estado |
| TC-FE-COR-050 | should not overwrite Authorization in sessionStorage when response has no auth header | Conserva `Authorization` previo si respuesta no trae uno nuevo | Estado |
| TC-FE-COR-051 | should handle 401 error without throwing | Respuesta 401 llega al subscriber de error sin romper | HTTP |
| TC-FE-COR-052 | should handle non-401 error without throwing | Respuesta 500 llega al subscriber de error sin romper | HTTP |
...
| ID | Descripción del test | Qué verifica | Tipo |
|----|---------------------|--------------|------|
| TC-FE-COR-053 | should add Bearer token header when a valid token exists | GET sale con `Authorization: Bearer <token>` | Seguridad |
| TC-FE-COR-054 | should pass request without Authorization when no token exists | Sin cookie, petición sin `Authorization` | Seguridad |
| TC-FE-COR-055 | should redirect to /login and throw error when token is expired | Token expirado → `err.message='Token expirado'`; navega a `/login`; borra cookie | Seguridad |
| TC-FE-COR-056 | should logout and redirect on 401 response | Backend devuelve 401 → navega a `/login`; elimina cookie | Seguridad |
| TC-FE-COR-057 | should rethrow non-401 errors without redirecting | Backend devuelve 500 → error propagado; no redirige | HTTP |
| TC-FE-COR-058 | should treat a malformed token as expired | `cookie='not-a-jwt'` → `err.message='Token expirado'`; navega a `/login` | Seguridad |
...
| ID | Descripción del test | Qué verifica | Tipo |
|----|---------------------|--------------|------|
| TC-FE-COR-059 | should be created | Instancia del manejador. | Creación |
| TC-FE-COR-060 | should log a string error to console | `handleError('Something went wrong')` → `console.error` invocado. | Estado |
| TC-FE-COR-061 | should log a generic Error to console | `new Error('Generic error')` → `console.error` invocado. | Estado |
| TC-FE-COR-062 | should handle HttpErrorResponse when offline | `navigator.onLine=false`, status 0 → mensaje `HTTP_ERRORES_CODIGO.NO_HAY_INTERNET`. | HTTP |
| TC-FE-COR-063 | should handle HttpErrorResponse with known status code and no mensaje | Status 404, sin mensaje en body → usa `PETICION_FALLIDA`. | HTTP |
| TC-FE-COR-064 | should handle HttpErrorResponse with error body containing mensaje | Status 500, `error.mensaje='Custom error'` → mensaje del backend conservado. | HTTP |
| TC-FE-COR-065 | should log error with date, path and message | Objeto logueado contiene `fecha`, `path` y `mensaje`. | Estado |
| TC-FE-COR-066 | should handle HttpErrorResponse with unknown status code | Status 999 → mensaje normalizado queda `undefined`. | HTTP |
...
| ID | Descripción del test | Qué verifica | Tipo |
|----|---------------------|--------------|------|
| TC-FE-COR-067 | should be created | Instancia del servicio. | Creación |
| TC-FE-COR-068 | should make a GET request via doGet | `doGet('/api/test', opts)` → respuesta `{id:1}`. | HTTP |
| TC-FE-COR-069 | should make a POST request via doPost | `doPost(url, body, opts)` → POST con body exacto. | HTTP |
| TC-FE-COR-070 | should make a PUT request via doPut | `doPut(url, body, opts)` → PUT con body exacto. | HTTP |
| TC-FE-COR-071 | should make a DELETE request via doDelete | `doDelete(url, opts)` → DELETE enviado. | HTTP |
| TC-FE-COR-072 | should make a GET request with params via doGetParameters | Con `HttpParams page=1` → GET con parámetro incluido. | HTTP |
| TC-FE-COR-073 | should set Content-Type header by default | `createDefaultOptions()` incluye `Content-Type: application/json`. | HTTP |
| TC-FE-COR-074 | should make a GET request by id via doGetById | `doGetById('/api/test/', 5, opts)` → GET a `/api/test/5`. | HTTP |
| TC-FE-COR-075 | should make a GET request by email via doGetByEmail | `doGetByEmail('/api/test/', 'user@test.com', opts)` → GET a `/api/test/user@test.com`. | HTTP |
| TC-FE-COR-076 | should make a POST without body via doPostWithOutBody | POST sin body a `/api/test/user@test.com`. | HTTP |
| TC-FE-COR-077 | should make a POST without body and id via doPostWithOutBodyAndId | POST a `/api/test/10`. | HTTP |
| TC-FE-COR-078 | should make a POST via doRequestMapping | `doRequestMapping('/api/test/', 7, opts)` → POST a `/api/test/7`. | HTTP |
| TC-FE-COR-079 | should make a PUT without body via doPutWithOutBody | PUT a `/api/test/3`. | HTTP |
| TC-FE-COR-080 | should set custom header via setHeader | `setHeader('X-Custom', 'valor')` agrega header y mantiene `Content-Type`. | HTTP |
| TC-FE-COR-081 | should use default options when doGet is called without opts | `doGet('/api/test')` sin opts usa GET válido. | HTTP |
| TC-FE-COR-082 | should handle doGetParameters with null params | `doGetParameters(url, null, opts)` → GET correcto. | Límite |
| TC-FE-COR-083 | should add Content-Type when opts headers lack it | `opts.headers = {Authorization: 'Bearer test'}` → `doGet` agrega `application/json`. | HTTP |
...
| ID | Descripción del test | Qué verifica | Tipo |
|----|---------------------|--------------|------|
| TC-FE-FEA-001 | should create | Instancia `LoginComponent`. | Creación |
| TC-FE-FEA-002 | should initialize the login form | `formularioLogin` y controles `userInput`/`passwordInput` creados en `ngOnInit`. | Validación |
| TC-FE-FEA-003 | should have required validators on userInput | `userInput=''` → control inválido. | Validación |
| TC-FE-FEA-004 | should have email validator on userInput | `userInput='not-email'` → `hasError('email')` es `true`. | Validación |
| TC-FE-FEA-005 | should accept valid email | `userInput='test@example.com'` → `hasError('email')` es `false`. | Validación |
| TC-FE-FEA-006 | should have required validator on passwordInput | `passwordInput=''` → control inválido. | Validación |
| TC-FE-FEA-007 | should clear error when userInput changes | `mensajeError` limpiado al cambiar `userInput`. | Estado |
| TC-FE-FEA-008 | should clear error when passwordInput changes | `mensajeError` limpiado al cambiar `passwordInput`. | Estado |
| TC-FE-FEA-009 | should not call service when form is invalid | Ambos campos vacíos → `validarLogin()` NO invocado. | Validación |
| TC-FE-FEA-010 | should call loginService with credentials when form is valid | Email/password válidos → `validarLogin({correo, contrasena})` invocado; `StateService` actualizado; navega a `/home`. | Navegación |
| TC-FE-FEA-011 | should set error message on login failure | `validarLogin` lanza `Error('Unauthorized')` → `mensajeError` contiene `'incorrectos'`. | Negativo |
...
| ID | Descripción del test | Qué verifica | Tipo |
|----|---------------------|--------------|------|
| TC-FE-FEA-012 | should be created | Instancia del servicio. | Creación |
| TC-FE-FEA-013 | should call GET to validate login | `validarLogin(...)` → GET a `/login`. | HTTP |
| TC-FE-FEA-014 | should store user details in sessionStorage when validating login | Antes de resolver, guarda JSON del login en `sessionStorage['userdetails']`. | Estado |
...
| ID | Descripción del test | Qué verifica | Tipo |
|----|---------------------|--------------|------|
| TC-FE-FEA-015 | should create | Instancia el componente. | Creación |
| TC-FE-FEA-016 | should load user types, identification types and structures | `init` llama a los 3 servicios de catálogos. | Estado |
| TC-FE-FEA-017 | should populate department, area and subarea lists | `listaDepartamentos/listaAreas/listaSubareas` con 1 elemento; `cargandoEstructuras=false`. | Estado |
| TC-FE-FEA-018 | should handle error loading structures | `cargandoEstructuras=false` ante error de `consultarDirecciones`. | Negativo |
| TC-FE-FEA-019 | should load user types | `tiposUsuario` cargados; `cargandoTiposUsuario=false`. | Estado |
| TC-FE-FEA-020 | should handle error loading user types | `errorTiposUsuario='Error al cargar los tipos de usuario'`. | Negativo |
| TC-FE-FEA-021 | should load identification types | `tiposIdentificacion` cargados; `cargandoTiposIdentificacion=false`. | Estado |
| TC-FE-FEA-022 | should handle error loading identification types | `errorTiposIdentificacion` recibe mensaje esperado. | Negativo |
| TC-FE-FEA-023 | should clear structure fields | `onTipoUsuarioChange()` limpia `tipoEstructura`, `estructuraOrganizacional`, `listaFiltrada`. | Estado |
| TC-FE-FEA-024 | should return false when no tipoUsuario selected | `esAdministradorDireccion()` → `false` con `tipoUsuario=''`. | Validación |
| TC-FE-FEA-025 | should return true when Administrador de dirección is selected | `tipoUsuario='tu2'` → `esAdministradorDireccion()` → `true`. | Validación |
| TC-FE-FEA-026 | should return false for Colaborador | `tipoUsuario='tu1'` → `esAdministradorDireccion()` → `false`. | Validación |
| TC-FE-FEA-027 | should set listaFiltrada to areas when tipo is area | `tipoEstructura='area'` → `listaFiltrada = listaAreas`. | Estado |
| TC-FE-FEA-028 | should set listaFiltrada to subareas when tipo is subarea | `tipoEstructura='subarea'` → `listaFiltrada = listaSubareas`. | Estado |
| TC-FE-FEA-029 | should clear listaFiltrada for unknown tipo | `tipoEstructura=''` → `listaFiltrada = []`. | Estado |
| TC-FE-FEA-030 | should return DIRECCION for admin | `determinarTipoArea()` → `'DIRECCION'` para admin. | Validación |
| TC-FE-FEA-031 | should return AREA when tipoEstructura is area | Para colaborador + `tipoEstructura='area'` → `'AREA'`. | Validación |
| TC-FE-FEA-032 | should return SUBAREA when tipoEstructura is subarea | Para colaborador + `tipoEstructura='subarea'` → `'SUBAREA'`. | Validación |
| TC-FE-FEA-033 | should return empty string when no match | Sin coincidencia → `determinarTipoArea()` → `''`. | Límite |
| TC-FE-FEA-034 | should return true when all fields are valid for collaborator | `validarFormulario()` → `true` con usuario completo. | Validación |
| TC-FE-FEA-035 | should return true for admin with estructura but no tipoEstructura | Admin con `estructuraOrganizacional` y `tipoEstructura=''` → `true`. | Validación |
| TC-FE-FEA-036 | should return false when missing required field | `nombres=''` → `validarFormulario()` → `false`. | Negativo |
| TC-FE-FEA-037 | should return false for collaborator without tipoEstructura | Colaborador sin `tipoEstructura` → `false`. | Negativo |
| TC-FE-FEA-038 | should set error when passwords do not match | `confirmarClave` distinto → `error='Las contraseñas no coinciden'`; NO llama `agregarNuevoUsuario`. | Negativo |
| TC-FE-FEA-039 | should set error when form is invalid | `nombres=''` → `error='Por favor, complete todos los campos requeridos'`. | Negativo |
| TC-FE-FEA-040 | should call service and notify on success | Servicio exitoso → `'Usuario creado exitosamente'`; emite evento `usuarioCreado`. | Positivo |
| TC-FE-FEA-041 | should handle error with error.mensaje | `{error:{mensaje:'Correo duplicado'}}` → muestra ese error. | Negativo |
| TC-FE-FEA-042 | should handle error with error.message | `{error:{message:'Bad request'}}` → muestra `'Bad request'`. | Negativo |
| TC-FE-FEA-043 | should handle generic error | `Error('unknown')` → usa mensaje genérico. | Negativo |
| TC-FE-FEA-044 | should reset all fields | `limpiarFormulario()` borra `nombres`, `error`, `mensajeExito`, `listaFiltrada`. | Estado |
| TC-FE-FEA-045 | should not throw when modal element not found | `cerrarModal()` con `getElementById=null` no lanza excepción. | Límite |
| TC-FE-FEA-046 | should set departamentos for admin type | Flujo con tipo admin no rompe. | Estado |
| TC-FE-FEA-047 | should emit usuarioCreado event on success | `registrarUsuario()` exitoso emite evento `usuarioCreado`. | Estado |
| TC-FE-FEA-048 | should hide modal and clean up backdrops | `cerrarModal()` con bootstrap.Modal llama `hide()` y limpia `modal-open` tras `tick(300)`. | Interfaz |
...
| ID | Descripción | Tipo |
|----|-------------|------|
| TC-FE-FEA-098 | should be created | Creación |
| TC-FE-FEA-099 | should emit when user is created | `usuarioCreado$` emite el objeto correcto | Estado |
| TC-FE-FEA-100 | should emit when user is updated | `usuarioActualizado$` emite el objeto correcto | Estado |
| TC-FE-FEA-101 | should emit when user is deleted | `usuarioEliminado$` emite el objeto correcto | Estado |
...
| ID | Descripción del test | Qué verifica | Tipo |
|----|---------------------|--------------|------|
| TC-FE-FEA-102 | should create | Instancia el componente. | Creación |
| TC-FE-FEA-103 | should start at step 1 | `currentStep=1` al iniciar. | Estado |
| TC-FE-FEA-104 | should have empty fields | `correo`, `codigo`, `nuevaContrasena` inician vacíos. | Estado |
| TC-FE-FEA-105 | should show error for empty email | `correo=''` → error sobre correo. | Negativo |
| TC-FE-FEA-106 | should show error for invalid email | `correo='not-an-email'` → error de correo inválido. | Negativo |
| TC-FE-FEA-107 | should call service with valid email | `correo='test@example.com'` → `solicitarCodigo('test@example.com')` invocado. | Positivo |
| TC-FE-FEA-108 | should show success message on success | `mensajeExito` contiene `'enviado'`; `enviandoCodigo=false`. | Positivo |
| TC-FE-FEA-109 | should handle 404 error | Status 404 → `mensajeError` contiene `'no'`. | Negativo |
| TC-FE-FEA-110 | should handle connection error | Status 0 → `mensajeError` contiene `'No se pudo conectar'`. | Negativo |
| TC-FE-FEA-111 | should show error when code is empty | `codigo=''` → mensaje de ingresar código. | Negativo |
| TC-FE-FEA-112 | should call service with code and email | `validarCodigo({codigo:'123456', correo:'test@example.com'})` invocado. | Positivo |
| TC-FE-FEA-113 | should show success on valid code | `validarCodigo=>{valor:true}` → mensaje de validación correcta. | Positivo |
| TC-FE-FEA-114 | should show error on invalid code | `validarCodigo=>{valor:false}` → `mensajeError` contiene `'incorrecto'`. | Negativo |
| TC-FE-FEA-115 | should handle 400 error with backend mensaje | Status 400 + `error.mensaje='Codigo expirado'` → muestra exactamente ese mensaje. | Negativo |
| TC-FE-FEA-116 | should show error when fields are empty | `cambiarContrasena()` con ambas claves vacías → mensaje de completar campos. | Negativo |
| TC-FE-FEA-117 | should show error when passwords do not match | `password1 ≠ password2` → `mensajeError` contiene `'no coinciden'`. | Negativo |
| TC-FE-FEA-118 | should show error when password is too short | `'12345'` → `mensajeError` contiene `'al menos 6 caracteres'`. | Negativo |
| TC-FE-FEA-119 | should call service with valid passwords | Clave válida → `recuperarClave({correo, clave:'newpass123'})` invocado. | Positivo |
| TC-FE-FEA-120 | should show success and redirect on success | `mensajeExito` contiene `'exitosamente'`. | Positivo |
| TC-FE-FEA-121 | should handle connection error on cambiarContrasena | Status 0 → `'No se pudo conectar'`. | Negativo |
| TC-FE-FEA-122 | should go back one step | `volverPaso()` reduce `currentStep 2→1`. | Estado |
| TC-FE-FEA-123 | should not go below step 1 | `currentStep=1` + `volverPaso()` → mantiene 1. | Límite |
| TC-FE-FEA-124 | should clear messages | `volverPaso()` limpia `mensajeError` y `mensajeExito`. | Estado |
| TC-FE-FEA-125 | should navigate to login | `volverLogin()` navega a `/login`. | Navegación |
| TC-FE-FEA-126 | should handle 400 error on enviarCodigo | Status 400 + mensaje `'Bad req'` → `mensajeError='Bad req'`. | Negativo |
| TC-FE-FEA-127 | should handle 500 error on enviarCodigo | Status 500 → `mensajeError` contiene `'Error del servidor'`. | Negativo |
| TC-FE-FEA-128 | should handle generic error on enviarCodigo | Status 503 → `mensajeError` contiene `'Error al solicitar'`. | Negativo |
| TC-FE-FEA-129 | should handle 0 error on validarCodigo | Status 0 → `'No se pudo conectar'`. | Negativo |
| TC-FE-FEA-130 | should handle 404 error on validarCodigo | Status 404 → `mensajeError` contiene `'expirado'`. | Negativo |
| TC-FE-FEA-131 | should handle 500 error on validarCodigo | Status 500 → `'Error del servidor'`. | Negativo |
| TC-FE-FEA-132 | should handle generic error on validarCodigo | Status 503 → `'Error al validar'`. | Negativo |
| TC-FE-FEA-133 | should handle 400 error on cambiarContrasena | Status 400 + `'Bad clave'` → `mensajeError='Bad clave'`. | Negativo |
| TC-FE-FEA-134 | should handle 404 error on cambiarContrasena | Status 404 → `'expirado'`. | Negativo |
| TC-FE-FEA-135 | should handle 500 error on cambiarContrasena | Status 500 → `'Error del servidor'`. | Negativo |
| TC-FE-FEA-136 | should handle generic error on cambiarContrasena | Status 503 → `'Error al cambiar'`. | Negativo |
...
| ID | Descripción | Tipo |
|----|-------------|------|
| TC-FE-FEA-137 | should be created | Creación |
| TC-FE-FEA-138 | should call POST to request recovery code → `/usuarios/recuperacion/solicitar/{correo}` | HTTP |
| TC-FE-FEA-139 | should call POST to validate recovery code → `/usuarios/recuperacion/validarCodigo` | HTTP |
| TC-FE-FEA-140 | should call PUT to recover password → `/usuarios/recuperacion/recuperarClave` | HTTP |
| TC-FE-FEA-141 | should propagate error from solicitarCodigo | HTTP |
| TC-FE-FEA-142 | should propagate error from validarCodigo y recuperarClave | HTTP |
...
|----|-------------|----------|------|
| TC-FE-FEA-190 | IndicatorService: should be created | — | Creación |
| TC-FE-FEA-191 | should call GET to consult indicators | `GET /indicadores` | HTTP |
| TC-FE-FEA-192 | should call GET to consult indicators for activities | `GET /indicadores/actividades` | HTTP |
| TC-FE-FEA-193 | should call POST to add a new indicator | `POST /indicadores` | HTTP |
| TC-FE-FEA-194 | should call PUT to modify an indicator | `PUT /indicadores/{id}` | HTTP |
| TC-FE-FEA-195 | IndicatorsService: should be created | — | Creación |
| TC-FE-FEA-196 | should call doGet for obtenerIndicadores | `doGet(url)` invocado | HTTP |
| TC-FE-FEA-197 | (additional mock test) | — | Estado |
...
| ID | Descripción | Tipo |
|----|-------------|------|
| TC-FE-SHR-136 | should create | Creación |
| TC-FE-SHR-137 | should accept icon input | `icon='fa-plus'` persistido | Renderizado |
| TC-FE-SHR-138 | should accept text input | `text='Add New'` persistido | Renderizado |
| TC-FE-SHR-139 | should accept modalTargetId input | `modalTargetId='testModal'` persistido | Renderizado |
| TC-FE-SHR-140 | should have empty defaults for icon and text | `icon` y `text` inician como `''` | Estado |
| TC-FE-SHR-141 | should have undefined modalTargetId by default | `modalTargetId` inicia `undefined` | Estado |
| TC-FE-SHR-142 | should call openModal without error when element does not exist | `openModal('non-existent-id')` no lanza excepción | Límite |
| TC-FE-SHR-143 | should open modal when element exists in DOM | Con `div.modal` en DOM, `openModal()` ejecuta sin romper | Interfaz |
...
| Feature: Gestión de Departamento | ~15 | ~40 |
...
| Efectividad de cobertura bytecode (backend) | ❌ Crítico | JaCoCo reporta ~2% — patrones `@Mock` incorrectos (ver Artefacto 34) |
...
|---------|-------|-------|-------------|
| 1.0.0 | 2026-03-27 | GitHub Copilot — Agente PO Ceiba | Creación inicial. Catalogación de ~1 028 casos de prueba extraídos directamente del código fuente mediante 2 subagentes de exploración. Backend: 135 clases Java (JUnit 5 + Mockito). Frontend: 120 spec.ts (Karma + Jasmine). Incluye trazabilidad hacia las 15 historias de usuario del Artefacto 16. |
