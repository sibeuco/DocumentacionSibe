...
6. **Consistencia entre backend y frontend:** El frontend guard `ROLE_ROUTES` es consistente con las restricciones del backend — las rutas `/gestionar-direccion`, `/gestionar-usuarios` y `/gestionar-indicadores` coinciden con endpoints que requieren `ADMINISTRADOR_DIRECCION_*`.
