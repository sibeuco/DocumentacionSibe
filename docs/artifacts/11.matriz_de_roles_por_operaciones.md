...
La **Matriz de Roles por Operaciones** documenta de forma exhaustiva qué operaciones puede ejecutar cada rol del sistema SIBE, tanto a nivel de API REST (backend) como a nivel de interfaz de usuario (frontend). Este artefacto se construye directamente del análisis del código fuente y constituye la referencia canónica de control de acceso.

**Fuentes de verdad utilizadas:**
- Constantes de seguridad: `SeguridadConstante.java`
- Anotaciones `@PreAuthorize` en cada controlador REST
- Configuración de Spring Security: `ProjectSecurityConfig.java`
- Data loader de roles: `DatosTipoUsuarioFabrica.java`
- Guards de rutas Angular: `security.guard.ts`, `public-route.guard.ts`
- Servicio de autorización contextual: `AutorizacionContextoOrganizacionalServicio.java`
...
|-----|--------|-------------|
| Administrador de Dirección | `ADMINISTRADOR_DIRECCION` | Gestión global del sistema. Acceso total a todas las áreas, subáreas y operaciones. |
| Administrador de Área | `ADMINISTRADOR_AREA` | Gestión de su área asignada y sus subáreas. Acceso de lectura a recursos globales. |
| Colaborador | `COLABORADOR` | Operador de actividades en su subárea. Acceso de lectura y ejecución de actividades. |

### 2.2 Privilegios CRUD por Rol (Data Loader)

Derivados de `DatosTipoUsuarioFabrica.java`, cada rol genera authorities compuestas `{ROL}_{OPERACION}`:
...
> ¹ El Colaborador solo puede modificar su propio perfil. La autorización contextual (`validarAccesoAUsuario`) verifica que `usuarioId == contexto.identificador`.
...
> ³ Aunque el `@PreAuthorize` permite los 3 roles, la autorización contextual (`AutorizacionContextoOrganizacionalServicio`) restringe la consulta al ámbito organizacional del usuario: el Admin. Área ve solo su área y subáreas; el Colaborador ve solo su subárea.
...
| `/gestionar-indicadores` | `securityGuard` + `ROLE_ROUTES` | ✅ | ✅ | ❌ → `/home` | Redirige a `/login` |
...
| **Área** | ✅ Cualquier área | ✅ Solo su área (`areaId == contexto.areaId`) | ❌ |
| **Subárea** | ✅ Cualquier subárea | ✅ Solo subáreas de su área | ✅ Solo su subárea (`subareaId == contexto.subareaId`) |
| **Actividad** | ✅ Cualquier actividad | ✅ Actividades de su área o subáreas de su área | ✅ Si es colaborador asignado o si la actividad pertenece a su subárea |
...
flowchart TD
    ...
    subgraph AA["ADMINISTRADOR_AREA"]
        AA1["¿Recurso pertenece a su Área?"]
        AA1 -->|"Sí"| AA2["✅ Acceso"]
        AA1 -->|"No"| AA3["¿Es subárea de su Área?"]
        AA3 -->|"Sí"| AA2
    ...
    subgraph CO["COLABORADOR"]
        ...
        CO1 -->|"Sí"| CO2["✅ Acceso"]
        CO1 -->|"No"| CO3["¿Recurso en su Subárea?"]
        CO3 -->|"Sí"| CO2
        CO3 -->|"No"| CO4["❌ 403 Forbidden"]
    end
```
...
| Gestionar Indicadores | `/gestionar-indicadores` | ✅ Visible | ✅ Visible | ❌ Oculto (guard) |
...
| Actividades (cualquier contexto) | Editar actividad | ✅ | ✅ | Solo campos de colaborador |
...
| Dashboard / Gráficas | Ver métricas globales (Dirección) | ✅ | ❌ (datos filtrados por backend) | ❌ |
| Dashboard / Gráficas | Ver métricas de área | ✅ | ✅ (solo su área) | ❌ |
| Dashboard / Gráficas | Ver métricas de subárea | ✅ | ✅ (subáreas de su área) | ✅ (solo su subárea) |
...
| | Consultar usuarios | ✅ | ✅ | ✅ |
| | Modificar usuario | ✅ (cualquiera) | ❌ | ✅ (solo a sí mismo) |
...
| **Indicadores** | Consultar indicadores | ✅ | ✅ (solo lectura en UI) | ✅ |
...
| | Consultar actividades | ✅ (todas) | ✅ (su área y subáreas) | ✅ (su subárea) |
| | Iniciar registro asistencia | ✅ | ✅ | ✅ |
| | Registrar asistencia (RFID/Doc/Externo) | ✅ | ✅ | ✅ |
| | Finalizar actividad | ✅ | ✅ | ✅ |
| | Cancelar actividad | ✅ | ✅ | ✅ |
| **Analítica** | Dashboard de métricas | ✅ (global) | ✅ (su área) | ✅ (su subárea) |
...
| `direccionId` | UUID de la dirección | ✅ (siempre presente) | Puede estar presente | ❌ (normalmente null) |
| `areaId` | UUID del área | ❌ (normalmente null) | ✅ (siempre presente) | ❌ (normalmente null) |
| `subareaId` | UUID de la subárea | ❌ (normalmente null) | ❌ (normalmente null) | ✅ (siempre presente) |
...
## 11. Observaciones y Hallazgos del Análisis

1. **Privilegio DELETE no utilizado para Admin. Área:** El data loader crea `ADMINISTRADOR_AREA_DELETE` pero ningún endpoint del backend referencia esta authority. El control de eliminación está restringido exclusivamente al Administrador de Dirección (`HAS_ADMIN_DELETE_AUTHORITY`).

2. **Privilegio CREATE del Colaborador sin uso directo:** El data loader genera `COLABORADOR_CREATE` pero ningún controlador usa `hasAuthority('COLABORADOR_CREATE')` de forma aislada. Las operaciones de escritura del Colaborador se canalizan mediante `COLABORADOR_UPDATE` (iniciar, finalizar, cancelar actividad, modificar clave).

3. **Doble capa de seguridad:** El sistema implementa 2 capas: (a) `@PreAuthorize` para validar la operación CRUD, y (b) `AutorizacionContextoOrganizacionalServicio` para validar el ámbito organizacional. Ambas deben pasar para que la operación sea exitosa.

4. **Frontend sin guard granular por subárea:** Las rutas de área y subárea (`/home/area-*`, `/home/.../sub-area-*`) son accesibles por cualquier usuario autenticado. La restricción de datos visible ocurre en el backend (autorización contextual) y en la lógica del frontend que envía el `areaId`/`subareaId` del token al hacer las peticiones.

5. **Endpoints estadísticos como POST:** Los endpoints de conteo y estadísticas (sec. 3.11) usan `POST` con `FiltroEstadisticaDTO` en el body, a pesar de ser consultas de datos. Esto es un patrón de diseño para transportar filtros complejos.

6. **Consistencia entre backend y frontend:** El frontend guard `ROLE_ROUTES` es consistente con las restricciones del backend — las rutas `/gestionar-direccion` y `/gestionar-usuarios` coinciden con endpoints que requieren `ADMINISTRADOR_DIRECCION_*`, y `/gestionar-indicadores` coincide con endpoints que requieren `ADMIN_*` o `AREA_ADMIN_*`.
