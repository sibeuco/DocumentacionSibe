...
| **Paquete Fuente**    | `co.edu.uco.sibe.dominio.modelo`                              |
| **Total de Clases**   | 32                                                            |
| **Fuente Principal**  | Código fuente Java del backend (`SIBEBackend/src/main/java/`) |
| **Fuente Secundaria** | `docs/deprecated_docs/Diagrama de Clases.drawio` (deprecado)  |
...
13. [Notas de Migración desde drawio](#13-notas-de-migración-desde-drawio)
...
El modelo de dominio de SIBE está compuesto por **32 clases** organizadas en el paquete `co.edu.uco.sibe.dominio.modelo`. Estas clases representan las entidades centrales del sistema de bienestar universitario, cubriendo desde la estructura organizacional de la universidad hasta el registro de asistencia a actividades mediante RFID.
...
classDiagram
    ...
    Direccion *-- "1..*" Area : contiene
    Direccion o-- "*" Actividad : asigna directamente
    Area *-- "1..*" Subarea : contiene
...
|---------------|----------|-------------------------------------------------------|
| identificador | `UUID`   | Identificador único                                   |
| nombre        | `String` | Nombre del estado (ej. Programada, Ejecutada, Cancelada)|
...
|----------------------|-----------------|--------------|-------------------------------------------------|
| `Direccion`          | `Area`          | `1..*`       | Una dirección contiene al menos un área          |
| `Area`               | `Subarea`       | `1..*`       | Un área contiene al menos una subárea            |
...
## 13. Notas de Migración desde drawio

El archivo `docs/deprecated_docs/Diagrama de Clases.drawio` contenía un diagrama de clases creado con draw.io/Electron v29.6.1. A continuación se documentan las observaciones de la migración:

| Aspecto                  | drawio (deprecado)                              | Mermaid (actual)                                |
|--------------------------|------------------------------------------------|-------------------------------------------------|
| **Formato**              | XML/draw.io propietario                        | Mermaid Markdown nativo                         |
| **Clases representadas** | Subconjunto parcial (~20 clases visibles)      | 32 clases completas desde código fuente         |
| **Campos**               | Algunos campos con placeholders vacíos         | Todos los campos validados contra código Java   |
| **Métodos**              | `+ method(type): type` genérico en todas       | Factory methods `construir()` reales documentados|
| **Relaciones**           | Composición/agregación con notación UML        | Preservadas con notación Mermaid equivalente    |
| **Typo identificado**    | `PublictoInteres` (error tipográfico en drawio) | Corregido a `PublicoInteres`                   |
| **Jerarquía Miembro**    | Incluida parcialmente                          | Completa con todas las subclases                |

### Relaciones del drawio preservadas

Las relaciones de composición (diamante lleno `◆`) y agregación (diamante vacío `◇`) del diagrama original fueron mappeadas fielmente:

- `endFill=1` (diamante lleno) → Composición (`*--`)
- `endFill=0` (diamante vacío) → Agregación (`o--`)
- `endArrow=block;endFill=0` → Herencia (`<|--`)
- `dashed=1;endArrow=open` → Dependencia (`..>`)
...
|-------------|---------|--------------------------------------------------------------------------|
| 2025-07-14  | 1.0     | Creación inicial — migración completa desde drawio + código fuente Java  |
