...
| Rol | Descripción |
| --- | ----------- |
| Administrador de Dirección | Responsable global de la Dirección. Gestiona usuarios, supervisa todas las áreas, configura indicadores estratégicos y consulta analítica consolidada. |
| Administrador de Área | Responsable de un área o subárea específica. Gestiona actividades, registra asistencia, carga participantes y consulta analítica de su ámbito. |
...
| 2 | Modificar datos | Administrador | Puede actualizar: nombre, objetivo, semestre, ruta de insumos, indicador, colaborador, estructura organizacional y fechas programadas. |
| 3 | Validar reglas | Sistema | Aplica las mismas validaciones que BP-10 (unicidad, semestre, autorización). |
| 4 | Persistir cambios | Sistema | Actualiza la actividad y sus ejecuciones (solo las que tienen estado "Pendiente" pueden modificarse). |

**Reglas de negocio:**

- RN-23: Solo se pueden modificar fechas de ejecuciones con estado "Pendiente".
- RN-24: Las ejecuciones finalizadas o canceladas no se pueden alterar.
...
| 4 | Consultar fechas programadas | Usuario | Selecciona una actividad para ver el modal con todas sus fechas programadas y sus estados (Pendiente, En Curso, Finalizada, Cancelada). |
...
| **Actores** | Administrador de Área, Colaborador (operador del kiosco). |
...
- RN-25: La búsqueda por RFID se realiza primero por ID de carnet; si no se encuentra, se intenta por número de identificación.
- RN-26: Un participante solo puede registrarse una vez por ejecución de actividad en el mismo semestre.
- RN-27: Los participantes internos se registran con una instantánea (snapshot) de sus datos al momento de la asistencia, de modo que cambios futuros en la base de miembros no afecten los registros históricos.
...
| **Actores** | Administrador de Área, Colaborador. |
...
- RN-28: Solo se puede finalizar una ejecución que está "En Curso".
- RN-29: La cancelación revierte al estado "Pendiente" (no la marca como "Cancelada"), permitiendo reiniciarla en otro momento.
- RN-30: Un mismo miembro solo puede tener un registro de asistencia por ejecución; intentos duplicados se rechazan.
...
- RN-31: Las métricas solo se calculan sobre ejecuciones con estado "Finalizada".
- RN-32: Los filtros dinámicos (años, meses, programas, centros de costos, etc.) se populan con valores reales extraídos de ejecuciones finalizadas existentes.
- RN-33: El indicador "Cobertura" requiere el cálculo de población total para generar el porcentaje.
...
stateDiagram-v2
    ...
    Pendiente --> Cancelada : Administrador cancela definitivamente
...
| Estado | Descripción |
| ------ | ----------- |
| **Pendiente** | Estado inicial de la ejecución. La fecha está programada pero no se ha iniciado. |
...
| **Cancelada** | La actividad fue cancelada definitivamente. No se registraron participantes. |
...
| **Ejecución de Actividad** | Instancia concreta de una actividad en una fecha específica. Tiene su propio ciclo de vida (Pendiente → En Curso → Finalizada/Cancelada). |
