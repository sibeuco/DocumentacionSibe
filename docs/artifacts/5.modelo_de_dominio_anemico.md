...
El modelo se organen **5 subdominios** que agrupan los objetos según su responsabilidad funcional:
...
classDiagram
    ...
    Persona "1..1" -- "0..*" Identificacion : tiene ▸
    ...
    UsuarioOrganizacion "1..*" -- "0..1" Area : pertenece ▸
    UsuarioOrganizacion "1..*" -- "0..1" Subarea : pertenece ▸
    ...
    Indicador "0..*" -- "1..1" PublicoInteres : tiene ▸
    Indicador "0..*" -- "1..1" Proyecto : pertenece ▸
    Proyecto "1..1" -- "0..*" Accion : tiene ▸
...
| Objeto | Tipo | Descripción |
| ------ | ---- | ----------- |
| **Miembro** | Entidad (Raíz de herencia) | Persona de la comunidad universitaria registrada en la base de datos SIBE. Se especializa en Externo o Interno. Contiene nombre completo, número de identificación e ID de carnet. |
...
| **Empleado** | Entidad (Especialización) | Interno vinculado laboralmente a la universidad. Contiene: clasificación, relación laboral y centro de costos. |
...
| Objeto | Tipo | Descripción |
| ------ | ---- | ----------- |
| **Indicador** | Entidad | Métrica estratégica que permite medir el impacto de las actividades. Se asocia con un tipo de indicador, una temporalidad, un público de interés y un proyecto del plan de desarrollo. |
...
| **EstadoActividad** | Catálogo / Enumeración | Estado de una ejecución: `Pendiente`, `En Curso`, `Finalizada` o `Cancelada`. |
...
| ------ | :----------: | ------- | :----------: | ----- | ----------- |
| Persona | 1..1 | Identificacion | 0..* | tiene | Una persona puede tener múltiples documentos de identificación. |
...
| UsuarioOrganizacion | 1..* | Area | 0..1 | pertenece a | Un usuario puede estar vinculado a nivel de Área. |
| UsuarioOrganizacion | 1..* | Subarea | 0..1 | pertenece a | Un usuario puede estar vinculado a nivel de Subárea. |
...
| Indicador | 0..* | PublicoInteres | 1..1 | tiene | Cada indicador apunta a un público de interés. |
| Indicador | 0..* | Proyecto | 1..1 | pertenece a | Cada indicador está vinculado a un proyecto del plan de desarrollo. |
| Proyecto | 1..1 | Accion | 0..* | tiene | Un proyecto agrupa múltiples acciones. |
