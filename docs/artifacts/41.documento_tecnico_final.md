# Artefacto 40 — Documento Técnico Final y Registro de Decisiones Arquitectónicas
...
| **Artefacto anterior** | [39. Manual de Operación del Producto](39.manual_de_operacion_del_producto.md) |
...
SIBE (Sistema de Información de Bienestar Estudiantil) es una **aplicación web empresarial** desarrollada para la Dirección de Bienestar y Evangelización de la Universidad Católica de Oriente (UCO) en Colombia. Su propósito es digitalizar la gestión de actividades de bienestar, el registro de asistencia de la comunidad universitaria, los indicadores estratégicos de gestión, y la generación de estadísticas institucionales.

El sistema reemplaza procesos manuales basados en planillas de papel, Google Forms, CheckPoint y AccuClass — herramientas que no cumplían con los requisitos de integración organizacional, trazabilidad estratégica ni generación automática de reportes.
...
| **Cobertura backend** | ~2% (causa raíz identificada: error estructural en pruebas) |
...
**Alternativas de mercado evaluadas y descartadas:** Google Forms, CheckPoint, AccuClass, planillas Excel — ninguna cumplía criterios de jerarquía organizacional nativa, registro por RFID, control de acceso contextual, carga masiva semestral, ni vinculación con el plan de desarrollo institucional.
...
- Gestión del ciclo de vida de actividades: PENDIENTE → EN_CURSO → FINALIZADA / CANCELADA.
...
- Proceso de matrícula y generación de constancias.
- Pagos en línea.
...
| **Administrador de Área** | Gestión de actividades e indicadores de su área | CRUD restringido a su área/subárea |
...
|------------|-----------|---------------|
| **Base de datos prod.** | PostgreSQL 15+ | Host: `localhost`, Puerto: `5432`, BD: `sibe_db2` |
...
| **Servidor frontend (prod)** | nginx | Sirve bundle estático; `try_files` para SPA routing |
...
C4Context
    ...
    Person(colab, "Colaborador", "Registro de asistencia, lectura y ejecución de actividades")

    System(sibe, "SIBE", "Sistema de Información de Bienestar Estudiantil — Gestión de actividades, asistencia, indicadores y analítica institucional de la UCO")
    ...
    Rel(adminDir, sibe, "Administra", "HTTPS / JWT Bearer")
    Rel(adminArea, sibe, "Opera", "HTTPS / JWT Bearer")
    Rel(colab, sibe, "Usa", "HTTPS / JWT Bearer")
    Rel(sibe, postgres, "Persiste / Consulta", "JDBC (JPA/Hibernate)")
    Rel(sibe, smtp, "Envía OTP", "SMTP / STARTTLS puerto 587")
    Rel(rfid, sibe, "Código de carnet", "USB/HID → input de teclado al navegador")
    Rel(excel, sibe, "Carga masiva", "Upload .xlsx vía multipart/form-data")
```
...
Los ADRs están documentados siguiendo el formato estándar de Michael Nygard, extendido con campos de implementación verificada y drivers trazados del Artefacto 19.
...
|-------|---------|
| **Estado** | ✅ Aceptado — Implementado en v1.0 |
| **Riesgo activo** | R-02 (secret hardcoded) — ver §7 |
...
**Generación de token:** `JWTTokenGeneratorFilter` captura el evento de autenticación exitoso y genera un JWT firmado con HMAC-SHA256 (algoritmo `HS256`) usando la clave `SeguridadConstante.JWT_KEY`. El JWT incluye los claims: `email`, `id`, `authorities`, `rol`, `direccionId`, `areaId`, `subareaId`. Expiración: 30,000,000 ms (~8.33 horas).
...
- Un JWT emitido es válido hasta su expiración (~8.33h) aunque el usuario sea desactivado (R-01).
- La clave de firma está hardcoded en código fuente (`SeguridadConstante.JWT_KEY`), lo cual es inaceptable en producción (R-02).
...
|-------|---------|
| **Estado** | ✅ Aceptado — Implementado en v1.0 |
| **Riesgo activo** | R-04 (password del admin en constante), R-05 (sin Flyway) |
...
| 9 | `DireccionDataLoader` | 1 dirección: BIENESTAR ESTUDIANTIL Y EVANGELIZACION |
...
- Sin versionado de schema (Flyway/Liquibase no implementado). Cambios en la BD que requieren migración de datos existentes deben hacerse manualmente (R-05).
- El password del admin seed está en `DatoConstante` en código fuente (R-04).
...
Se requiere importar semestralmente miles de registros de estudiantes y empleados desde archivos `.xlsx` proporcionados por sistemas académicos de la UCO. El procesamiento de estudiantes y el de empleados comparten la lógica de lectura del workbook peo difieren en el mapeo de columnas a objetos de comando.
...
### ADR-013: CORS Restringido y API Stateless sin CSRF

| Campo | Detalle |
|-------|---------|
| **Estado** | ✅ Aceptado — Implementado v1.0 · ⚠️ Requiere actualización para producción |
| **Riesgo activo** | R-06 (CORS hardcoded a localhost) |
...
**CORS:** Configurado en `ProjectSecurityConfig` para permitir solo `http://localhost:4200` (origen del frontend en desarrollo):
...
- `SeguridadConstante.LOCAL_FRONT_URL = "http://localhost:4200"` está hardcoded. En despliegue productivo debe externalizarse a variable de entorno (R-06).
...
|------------|--------------|-----------------|-----------|--------|
| **Backend (JaCoCo)** | 1.89% (407/21.493) | 2.96% (33/1.113) | 2.98% (48/1.609) | 2.09% (105/5.019) |
| **Frontend (Istanbul)** | 98.30% (4.229/4.302) | 87.86% (1.462/1.664) | 98.54% (1.082/1.098) | 98.53% (4.096/4.157) |

**Causa raíz de la baja cobertura backend:** Las clases de prueba usan `@Mock` sobre las clases bajo prueba en lugar de `@InjectMocks`, lo que impide que JaCoCo registre ejecución del código de producción. Esta es deuda técnica crítica documentada en el Artefacto 34, con plan de remediación de 4 sprints.
...
|----|--------|-----------|--------|-----------|
| **R-01** | JWT irrevocable — token válido ~8.33h aunque usuario sea desactivado | Media | Aceptado | Las operaciones que consultan datos del usuario fallan para usuarios inactivos; el logout del cliente destruye el token almacenado |
| **R-02** | Secret JWT hardcoded en `SeguridadConstante.JWT_KEY` | **Crítica** | **Abierto** | Externalizar a variable de entorno (`JWT_SECRET`) antes de despliegue productivo |
| **R-03** | Credenciales SMTP (correo y App Password) en `application.properties` | **Crítica** | **Abierto** | Externalizar a variables de entorno o Vault antes de despliegue |
| **R-04** | Password del admin seed en `DatoConstante` en código fuente | Alta | Abierto | Cambiar manualmente tras primer despliegue; externalizar valor en DataLoader |
| **R-05** | Sin migraciones de BD versionadas (Flyway/Liquibase no implementado) | Alta | Abierto | Implementar Flyway para entorno de producción; `ddl-auto=validate` en prod |
| **R-06** | CORS hardcoded a `http://localhost:4200` en `SeguridadConstante` | Alta | Conocido | Externalizar a variable de entorno `CORS_ALLOWED_ORIGIN` |
| **R-07** | Context-path con punto extra en `application.properties` (`/api.`) | Media | Potencial | Verificar configuración en runtime; corregir a `server.servlet.context-path=/api` |
| **R-08** | Sin límite de tamaño de archivo (`max-file-size`) en Spring Boot | Media | Abierto | Agregar `spring.servlet.multipart.max-file-size=50MB` en properties |
| **R-09** | Acumulación de peticiones OTP expiradas en BD | Baja | Menor | Las filas se sobreescriben al re-solicitar; definir proceso batch de limpieza periódica |
...
| **R-11** | Frontend envía `X-XSRF-TOKEN` innecesariamente | Baja | Sin impacto | Header ignorado por el backend; limpiar en próxima versión del interceptor |
| **R-12** | Formato de archivo Excel UCO puede cambiar semestralmente | Media | Mitigado parcialmente | El motor Excel valida cabeceras; fallo controlado con `IllegalStateException`; documentar columnas esperadas |
...
Estos requerimientos tienen impacto directo en la estructura arquitectónica del sistema. Su implementación llevó a las decisiones documentadas en §5.
...
Las siguientes acciones deben realizarse **antes de despliegue en producción** (por orden de prioridad):

**Prioridad CRÍTICA:**

1. **Externalizar el secret JWT:**
   ```properties
   # application.properties
   app.security.jwt-key=${JWT_SECRET}
   # En servidor: export JWT_SECRET="clave-aleatoria-de-256-bits-minimo"
   ```
   En producción nunca debe existir un valor por defecto. Usar mínimo 256 bits de entropía.

2. **Externalizar credenciales SMTP:**
   ```properties
   spring.mail.username=${SMTP_USER}
   spring.mail.password=${SMTP_PASSWORD}
   ```

3. **Externalizar el CORS origin:**
   ```properties
   app.cors.allowed-origin=${CORS_ORIGIN:http://localhost:4200}
   ```

4. **Cambiar el password del admin seed:**
   Después del primer despliegue, el Administrador de Dirección debe cambiar su contraseña inmediatamente desde la interfaz de cambio de contraseña.

**Prioridad ALTA:**

5. **Implementar Flyway para gestión de schema:**
   ```groovy
   // build.gradle
   implementation 'org.flywaydb:flyway-core'
   ```
   Cambiar `ddl-auto=validate` en producción. Las migraciones se versionan en `src/main/resources/db/migration/`.

6. **Configurar tamaño máximo de archivo para carga masiva:**
   ```properties
   spring.servlet.multipart.max-file-size=50MB
   spring.servlet.multipart.max-request-size=50MB
   ```

7. **Corregir la estructura de pruebas backend:**
   Cambiar `@Mock` por `@InjectMocks` en todas las clases de prueba que instancian la clase bajo prueba. Esto elevará la cobertura de ~2% a los niveles esperados.

8. **Corregir el context-path en application.properties:**
   Verificar que `server.servlet.context-path=/api` (sin el punto extra que genera R-07).
...
| Agregar `LimitRequestSize` filter para multipart | Proteger el endpoint de carga masiva | Bajo |
...
gantt
    ...
    Flyway para gestión de schema        :crit, 2026-04, 2w
    Corregir pruebas backend             :crit, 2026-04, 3w
    Límite de tamaño de archivo          :crit, 2026-04, 1w
    section Calidad
...
|---------|-------|-------|-------------|
| 1.0.0 | 2026-03-27 | GitHub Copilot — Agente PO Método Ceiba | Creación inicial del Documento Técnico Final como baseline del sistema SIBE v1.0. Basado en análisis exhaustivo de: código fuente backend (645 clases), código fuente frontend (~122 componentes/servicios), Artefacto 19 (Drivers Arquitectónicos — 18 decisiones, 14 atributos de calidad, 15 restricciones, 12 riesgos documentados), Artefacto 21 (Arquitectura de Referencia), Artefacto 22 (Arquetipo de Solución), Artefacto 23 (Stack Tecnológico), Artefacto 33 (Análisis de Calidad y Deuda Técnica), Artefacto 34 (Cobertura de Código), estándares de codificación (coding-standards.md) y memoria de arquitectura del repositorio. Incluye 18 ADRs numerados en formato Nygard extendido, métricas de calidad actualizadas, inventario de deuda técnica, 12 riesgos arquitectónicos, 15 restricciones técnicas, 10 requerimientos funcionalmente significativos, matriz de trazabilidad ADR↔Driver, y roadmap técnico de evolución post-v1.0. |
