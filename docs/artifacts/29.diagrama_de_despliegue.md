...
| **Fuente**            | `application.properties`, `build.gradle`, `angular.json`, `proxy.conf.json`, `environment.ts`, `ProjectSecurityConfig.java` |
...
## 3. Topología de Despliegue — Producción (Propuesta)

### 3.1 Descripción

Arquitectura propuesta para despliegue en producción con separación de nodos, reverse proxy, y comunicación segura.
...
flowchart TB
    ...
    subgraph DMZ["🔒 Zona Desmilitarizada (DMZ)"]
        subgraph NGINX_NODE["Servidor Web / Reverse Proxy"]
            NGINX["nginx<br/>───────────<br/>Puerto: 443 (HTTPS)<br/>Puerto: 80 (redirect → 443)<br/>───────────<br/>Sirve: dist/sibe-frontend/<br/>Proxy: /api → backend:8080<br/>SSL Termination<br/>Gzip Compression<br/>Cache de estáticos"]
        end
    end

    subgraph APPZONE["🏢 Zona de Aplicación"]
        subgraph APP_NODE["Servidor de Aplicación"]
            JVM_PROD["JVM OpenJDK 17<br/>───────────<br/>sibe-0.0.1-SNAPSHOT.jar<br/>Puerto: 8080<br/>Context Path: /api<br/>Session: STATELESS<br/>Memoria: -Xmx512m<br/>Spring Profile: prod"]
        end
    end

    subgraph DATAZONE["🗄️ Zona de Datos"]
        subgraph DB_NODE["Servidor de Base de Datos"]
            PGPROD["PostgreSQL 15+<br/>───────────<br/>Base: sibe_db2<br/>Puerto: 5432<br/>SSL Mode: require<br/>Connection Pool<br/>Backups automáticos"]
        end
    end
    ...
    CLIENT -->|"HTTPS :443<br/>TLS 1.2+"| NGINX
    NGINX -->|"HTTP :8080<br/>(red interna)"| JVM_PROD
    JVM_PROD -->|"JDBC :5432<br/>(red interna, SSL)"| PGPROD
    JVM_PROD -->|"SMTP :587<br/>(STARTTLS)"| SMTP_PROD

    style DMZ fill:#e8f5e9,stroke:#388e3c,stroke-width:2px
    style APPZONE fill:#e3f2fd,stroke:#1565c0,stroke-width:2px
    style DATAZONE fill:#fce4ec,stroke:#c62828,stroke-width:2px
...
|--------------------------------|----------------|--------------------------|----------------------------|-----------------|
| `sibe-0.0.1-SNAPSHOT.jar`     | Fat JAR        | `./gradlew bootJar`     | Servidor de Aplicación     | ~50-80 MB       |
| `dist/sibe-frontend/`         | Directorio     | `ng build --production` | Servidor Web (nginx)       | ~2-5 MB         |
| `application.properties`       | Config         | Manual / CI              | Dentro del JAR o externo   | ~1 KB           |
| `environment.ts`               | Config         | Manual / CI              | Compilado en `main.js`     | ~200 B          |
| Esquema DDL                    | SQL implícito  | Hibernate Auto DDL       | PostgreSQL                 | N/A             |
...
| Propiedad              | Desarrollo (Local)                     | Pruebas (Test)                   | Producción (Propuesto)                |
|------------------------|----------------------------------------|----------------------------------|---------------------------------------|
| **Frontend Server**    | `ng serve` (webpack-dev-server :4200)  | ChromeHeadless (Karma)           | nginx :443 (HTTPS)                    |
| **Backend Server**     | Tomcat embebido :8080                  | Spring context (test slice)      | Tomcat embebido :8080 tras proxy      |
| **Base de Datos**      | PostgreSQL local :5432                 | H2 in-memory                     | PostgreSQL dedicado :5432 (SSL)       |
| **DDL Strategy**       | `update` (auto-genera)                 | `create-drop` (recrea cada test) | `validate` o `none` + migraciones     |
| **CORS Origin**        | `http://localhost:4200`                | N/A                              | `https://sibe.edu.co` (dominio prod)  |
| **SSL/TLS**            | No                                     | No                               | Sí (nginx SSL termination)            |
| **JWT Key**            | Clave hardcoded en constante           | Misma clave (test)               | Variable de entorno / vault           |
| **SMTP**               | Gmail SMTP (smtp.gmail.com:587)        | Mock / deshabilitado             | SMTP institucional o Gmail            |
| **Logging**            | Console (INFO)                         | Console (DEBUG)                  | File + centralized (ELK/similar)      |
| **Profile Spring**     | default                                | `test` (`@ActiveProfiles`)       | `prod` (propuesto)                    |
...
|-------------|---------|----------------------------------------------------------------------------------------------|
| 2026-03-26  | 1.0     | Creación inicial — Diagrama de despliegue completo con topologías desarrollo y producción    |
