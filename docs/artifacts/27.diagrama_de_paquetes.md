...
|----------------------------------------|-------------------|-----------------------|
| **Archivos fuente (producción)**       | 651               | 387                   |
...
graph TB
    subgraph root["co.edu.uco.sibe (1)"]
        ...
        subgraph infraestructura["📦 infraestructura — Capa de Infraestructura"]
            ...
            infRepCmd["📦 adaptador.repositorio.comando (19)"]
...
| 12| `co.edu.uco.sibe.dominio.puerto.comando`                              | 19     | Puertos de escritura (interfaces)        |
...
| 29| `co.edu.uco.sibe.infraestructura.adaptador.repositorio.comando`       | 19     | Implementaciones de puertos comando      |
...
|   | **Total paquetes con código**                                         | **40** | **651 clases**                           |
...
graph TB
    subgraph dominio["📦 co.edu.uco.sibe.dominio"]
        ...
        subgraph puertos["Puertos (Interfaces)"]
            ptoCmd["📦 puerto.comando (19)<br/><i>XxxRepositorioComando</i>"]
...
| `puerto.comando`     | 19       | Contratos de escritura hacia persistencia                                   |
...
graph TB
    subgraph infraestructura["📦 co.edu.uco.sibe.infraestructura"]
        subgraph adaptador["📦 adaptador — Persistencia"]
            ...
            repCmd["📦 repositorio.comando (19)<br/><i>@Repository impl Cmd</i>"]
...
| `adaptador.repo.comando`  | 19       | Implementación de puertos de escritura del dominio           |
...
graph LR
    subgraph Infraestructura
        ...
        repo["adaptador.repositorio<br/>(43 clases)"]
    ...
    subgraph Dominio
        ...
        puertos["puerto<br/>(46 interfaces)"]
...
```mermaid
graph LR
    subgraph Producción["src/main/java (651 archivos)"]
        pApp["aplicacion<br/>(139)"]
        pDom["dominio<br/>(267)"]
        pInf["infraestructura<br/>(244)"]
    end
...
| `infraestructura.adaptador.repositorio.comando`           | 19    | 19 repos (100%)                          |
...
| `feature/home/`              | `ActivityService`            | `controlador.consulta.ActividadConsultaControlador`|
| `feature/manage-department/` | `DepartmentService`          | `controlador.consulta.DireccionConsultaControlador`|
...
| `feature/manage-indicators/` | `IndicatorService`, `ProjectService`, `ActionService` | `controlador.comando.IndicadorComandoControlador` + `controlador.consulta.IndicadorConsultaControlador` + análogos |
| `shared/service/`            | `AreaService`, `SubAreaService`, `UserTypeService`, `IdentificationTypeService` | Controladores consulta de catálogos |
...
| Puertos (todas las interfaces) | `dominio.puerto.*`                     | 46       |
...
| Repositorios implementación     | `infraestructura.adaptador.repositorio`| 43       |
...
| **Encapsulamiento de Dominio**  | 267 clases en `dominio` sin dependencia de Spring ni JPA                  |
...
|-------------|---------|--------------------------------------------------------------------------------------------|
| 2026-03-26  | 1.0     | Creación inicial — inventario completo de 51 paquetes backend (651 clases) y ~80 directorios frontend (387 archivos TS) desde código fuente |
