...
Este artefacto documenta los **flujos de transacciones** del sistema SIBE (Sistema de Información de Bienestar Estudiantil) mediante diagramas de actividades modelados en sintaxis **Mermaid (`flowchart TD`)**. Cada diagrama traza el camino completo de una operación de comando desde la entrada HTTP hasta la persistencia en base de datos, incluyendo:
...
Los diagramas se derivan directamente del **código fuente** del backend (Java/Spring Boot, arquitectura hexagonal, CQRS) y del frontend (Angular), complementados con la documentación existente del proyecto.
...
flowchart TD
    ...
    subgraph UC["Dominio - Caso de Uso"]
        ...
        REPO1 --> VINC["VincularUsuarioConAreaService.ejecutar\n(identificador, areaId, tipoArea)\nPersistir UsuarioOrganizacion\ncaso DIRECCION: vincularConDireccion\ncaso AREA: vincularConArea\ncaso SUBAREA: vincularConSubarea"]
    end
...
```mermaid
flowchart TD
    A([INICIO]) --> B[PUT /proyectos/{id}\nPreAuthorize: ADMIN_DIRECCION_UPDATE\no ADMIN_AREA_UPDATE]
...
flowchart TD
    ...
    P --> Q{¿Nombre único\nen semestre y área?}
    Q -- Sí --> ERR4[/Lanzar excepción\nactividad duplicada/]
    ERR4 --> Z4([FIN - Error 409])
    Q -- No --> R[actividadDAO.save Actividad]
...
flowchart TD
    ...
    N --> O{¿Nombre único\nen semestre y área?}
    O -- Sí --> ERR4[/Lanzar excepción\nactividad duplicada/]
    ERR4 --> Z4([FIN - Error 409])
    O -- No --> P[actividadDAO.findById + save\nActualizar campos]
...
### 7.4 Hallazgos Notables de Implementación

Los siguientes hallazgos se identificaron durante la trazabilidad del código fuente y representan desviaciones respecto al comportamiento esperado o áreas de mejora potencial:

| # | Hallazgo | Ubicación | Impacto |
|---|----------|-----------|---------|
| 1 | **Login usa GET en lugar de POST** | `LoginControlador`, `SecurityFilterChain` | Las credenciales viajan en la URL (cabecera Basic mitigada por HTTPS), pero GET no es idempotente para autenticación según RFC 7231 |
| 2 | **`ModificarActividadUseCase` usa `TipoOperacion.CREAR` en lugar de `ACTUALIZAR`** | `ModificarActividadUseCase.ejecutar()` | Las reglas del motor aplicadas en actualización son las de creación; puede omitir validaciones específicas de actualización |
| 3 | **`CancelarActividad` no tiene estado "CANCELADA"** | `CancelarActividadUseCase` | La cancelación revierte a `PENDIENTE` en lugar de marcar como `CANCELADA`, lo que dificulta la auditoría de cancelaciones |
| 4 | **Recuperar Clave no re-verifica el código de recuperación** | `RecuperarClaveUseCase` | El tercer paso del flujo confía en que los pasos anteriores fueron ejecutados; cualquier POST directo al endpoint podría cambiar la clave |
| 5 | **`MOTOR_USUARIO` en actualización no revalida patrón de clave** | `ModificarUsuarioUseCase` | Al modificar datos del usuario (sin cambiar clave), el motor ACTUALIZAR podría no aplicar todas las reglas de formato |
| 6 | **`ModificarProyectoUseCase` construye con `numeroProyecto` vacío** | `ProyectoFabrica.construirActualizar()` | El campo se inicializa con cadena vacía al construir la actualización |
