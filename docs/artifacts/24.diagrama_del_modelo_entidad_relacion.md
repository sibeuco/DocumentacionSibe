...
| **Artefacto anterior** | [23. Stack Tecnológico](23.stack_tecnologico.md) |
| **Artefacto siguiente** | [25. Diagrama de Componentes](25.diagrama_de_componentes.md) |
...
> **Nota técnica:** Todas las claves primarias (`identificador`) son de tipo `UUID` generado por la aplicación. El esquema relacional es gestionado automáticamente por Hibernate con `ddl-auto=update`. Las tablas `*_join*` con nombres compuestos (ej: `proyecto_accion`, `indicador_publico_interes`) representan entidades de relación M:N materializadas como entidades propias en el modelo de dominio.
...
erDiagram
    ...
    USUARIO ||--o{ USUARIO_ORGANIZACION : "pertenece a"
    DIRECCION ||--o{ USUARIO_ORGANIZACION : "agrupa"
    AREA ||--o{ USUARIO_ORGANIZACION : "agrupa"
    SUBAREA ||--o{ USUARIO_ORGANIZACION : "agrupa"
    TIPO_USUARIO ||--o{ USUARIO : "clasifica"

    ACTIVIDAD ||--o{ EJECUCION_ACTIVIDAD : "genera"
    ...
    ACTIVIDAD }o--|| AREA : "pertenece a"
    ACTIVIDAD }o--|| SUBAREA : "asignada a"

    PARTICIPANTE }o--|| MIEMBRO : "identifica"
```
...
erDiagram
    ...
    area ||--o{ usuario_organizacion : "asigna usuarios"
    subarea ||--o{ usuario_organizacion : "asigna usuarios"
...
| `area` → `subarea` | 1:N | Un área tiene cero o muchas subáreas |
| `direccion` → `usuario_organizacion` | 1:N | Una dirección agrupa muchos registros de asignación |
| `area` → `usuario_organizacion` | 1:N | Un área agrupa muchos registros de asignación |
| `subarea` → `usuario_organizacion` | 1:N | Una subárea agrupa muchos registros de asignación |
| `usuario_organizacion` → `usuario` | N:1 | Cada asignación referencia exactamente un usuario |
...
erDiagram
    ...
    tipo_usuario ||--|| usuario_tipo_usuario : "define"
    usuario_tipo_usuario ||--|{ usuario : "asignado a"
    usuario ||--|| persona : "tiene datos personales"
    persona ||--|| identificacion : "tiene"
    tipo_identificacion ||--|| identificacion_tipo_identificacion : "referenciada por"
    identificacion_tipo_identificacion ||--o{ identificacion : "describe"
```
...
| Relación | Cardinalidad | Descripción |
|----------|-------------|-------------|
| `tipo_usuario` → `usuario_tipo_usuario` | 1:1 | Cada tipo de usuario tiene su registro de clasificación |
| `usuario_tipo_usuario` → `usuario` | 1:N | Un tipo puede asignarse a muchos usuarios |
...
| `tipo_identificacion` → `identificacion_tipo_identificacion` | 1:1 | Referencia al tipo de documento |
| `identificacion_tipo_identificacion` → `identificacion` | 1:N | Un tipo puede estar en muchos documentos |
...
erDiagram
    ...
    estado_actividad ||--|{ ejecucion_actividad_estado_actividad : "define"
...
erDiagram
    ...
    participante {
        UUID identificador PK
        UUID miembro FK
        BIT es_participante_interno
        BIT es_participante_externo
    }
    ...
    participante ||--o{ participante_interno : "especializa"
    participante ||--o{ participante_externo : "especializa"
    participante_interno ||--o{ participante_estudiante : "especializa"
    participante_interno ||--o{ participante_empleado : "especializa"
    participante_interno }o--|| ciudad_residencia : "reside en"
    participante_empleado }o--|| relacion_laboral : "tiene"
    participante_empleado }o--|| centro_costos : "pertenece a"
```
...
erDiagram
    ...
    ciudad_residencia ||--|| interno_ciudad_residencia : "referenciada por"
    empleado ||--|| empleado_relacion_laboral : "tiene"
    relacion_laboral ||--|| empleado_relacion_laboral : "referenciada por"
    empleado ||--|| empleado_centro_costos : "tiene"
    centro_costos ||--|| empleado_centro_costos : "referenciada por"
```
...
erDiagram
    ...
    proyecto ||--o{ proyecto_accion : "comprende"
    accion ||--|| proyecto_accion : "forma parte de"
    proyecto ||--o{ indicador : "tiene"
    tipo_indicador ||--|| indicador_tipo_indicador : "tipifica"
    indicador_tipo_indicador ||--|{ indicador : "clasifica"
    temporalidad ||--|| indicador_temporalidad : "define"
    indicador_temporalidad ||--|{ indicador : "periodicidad"
    proyecto ||--|| indicador_proyecto : "vincula"
    indicador_proyecto ||--|{ indicador : "pertenece a"
    publico_interes ||--o{ indicador_publico_interes : "referenciado en"
    indicador ||--o{ indicador_publico_interes : "aplica a"
```
...
| `temporalidad` | Diaria, Semanal, Mensual, Trimestral, Anual |
...
|-------|---------|------|-------------|
| PK | `identificador` | UUID | Identificador único |
| | `nombre` | VARCHAR(15) | Nombre del estado: `PENDIENTE`, `EN_CURSO`, `FINALIZADA`, `CANCELADA` |
...
|-------|---------|------|-------------|
| PK | `identificador` | UUID | Identificador único |
| | `nombre` | VARCHAR(30) | Nombre del tipo (ej: ADMINISTRADOR_AREA) |
...
|-------|---------|------|-------------|
| PK | `identificador` | UUID | Identificador único |
| | `nombre` | VARCHAR(50) | Nombre de la subárea |
...
| | `es_participante_interno` | BIT | Indica si es miembro interno (UCO) |
| | `es_participante_externo` | BIT | Indica si es externo a la UCO |
...
| FK | `proyecto` | UUID | Proyecto estratégico al que pertenece |
...
|-------|---------|------|-------------|
| PK | `identificador` | UUID | Identificador único |
| | `nombre` | VARCHAR(30) | Nombre: Diaria, Semanal, Mensual, Trimestral, Anual |
...
| 2 | `tipo_usuario` | `\|\|` | `\|\|` | `usuario_tipo_usuario` | Un tipo de usuario tiene un registro de tipo |
| 3 | `usuario` | `\|\|` | `\|\|` | `persona` | Un usuario tiene exactamente una persona |
| 4 | `persona` | `\|\|` | `\|\|` | `identificacion` | Una persona tiene exactamente un documento |
...
| 8 | `direccion` | `\|\|` | `o{` | `usuario_organizacion` | Una dirección agrupa cero o muchas asignaciones |
| 9 | `direccion` | `\|\|` | `o{` | `area` | Una dirección tiene cero o muchas áreas |
| 10 | `identificacion_tipo_identificacion` | `\|\|` | `o{` | `identificacion` | Un tipo aplica a cero o muchos documentos |
...
| 14 | `ejecucion_actividad` | `o{` | `\|\|` | `actividad` | Cero o muchas ejecuciones pertenecen a una actividad |
| 15 | `ejecucion_actividad_estado_actividad` | `\|\|` | `\|{` | `ejecucion_actividad` | Registro de estado para una o muchas ejecuciones |
| 16 | `estado_actividad` | `\|\|` | `\|\|` | `ejecucion_actividad_estado_actividad` | Un estado define exactamente un registro |
| 17 | `tipo_identificacion` | `\|\|` | `\|\|` | `identificacion_tipo_identificacion` | Un tipo forma exactamente un registro |
| 18 | `usuario_tipo_usuario` | `\|\|` | `\|{` | `usuario` | Un registro de tipo aplica a uno o muchos usuarios |
| 19 | `registro_asistencia` | `o{` | `o{` | `ejecucion_actividad` | Cero o muchos registros por ejecución |
| 20 | `registro_asistencia` | `o{` | `o\|` | `participante` | Cero o muchos registros por participante |
...
| 24 | `ciudad_residencia` | `\|\|` | `\|\|` | `interno_ciudad_residencia` | Una ciudad tiene un registro de vinculación |
| 25 | `interno_ciudad_residencia` | `\|\|` | `\|{` | `interno` | Un registro de ciudad aplica a uno o muchos internos |
| 26 | `accion` | `\|\|` | `\|\|` | `proyecto_accion` | Una acción tiene un registro de vinculación |
| 27 | `proyecto_accion` | `\|\|` | `\|{` | `proyecto` | Un registro vincula a uno o muchos proyectos |
| 28 | `tipo_indicador` | `\|\|` | `\|\|` | `indicador_tipo_indicador` | Un tipo tiene un registro de vinculación |
| 29 | `indicador_tipo_indicador` | `\|\|` | `\|{` | `indicador` | Un registro clasifica a uno o muchos indicadores |
| 30 | `temporalidad` | `\|\|` | `\|\|` | `indicador_temporalidad` | Una temporalidad tiene un registro |
| 31 | `indicador_temporalidad` | `\|\|` | `\|{` | `indicador` | Un registro aplica a uno o muchos indicadores |
| 32 | `proyecto` | `\|\|` | `\|\|` | `indicador_proyecto` | Un proyecto tiene un registro de vinculación |
| 33 | `indicador_proyecto` | `\|\|` | `\|{` | `indicador` | Un registro pertenece a uno o muchos indicadores |
| 34 | `publico_interes` | `\|\|` | `o{` | `indicador_publico_interes` | Un público está en cero o muchos indicadores |
| 35 | `indicador_publico_interes` | `\|\|` | `\|{` | `indicador` | Un registro vincula a uno o muchos indicadores |
| 36 | `participante` | `\|\|` | `\|\|` | `miembro` | Un participante identifica exactamente un miembro |
| 37 | `participante` | `\|\|` | `\|\|` | `participante_externo` | Un participante puede ser externo |
| 38 | `participante_interno` | `\|\|` | `\|\|` | `participante` | Un participante interno referencia su base |
| 39 | `participante_interno` | `\|\|` | `\|\|` | `ciudad_residencia` | Un participante interno tiene ciudad de residencia |
| 40 | `participante_estudiante` | `\|\|` | `\|\|` | `participante_interno` | Un est. referencia su participante interno |
| 41 | `participante_empleado` | `\|\|` | `\|\|` | `participante_interno` | Un empl. referencia su participante interno |
| 42 | `externo` | `\|\|` | `\|\|` | `miembro` | Un externo tiene exactamente un miembro base |
| 43 | `relacion_laboral` | `\|\|` | `\|\|` | `empleado_relacion_laboral` | Una relación laboral tiene su registro |
| 44 | `empleado_relacion_laboral` | `\|\|` | `\|{` | `empleado` | Un registro aplica a uno o muchos empleados |
| 45 | `empleado_relacion_laboral` | `\|\|` | `\|{` | `participante_empleado` | Un registro aplica a uno o muchos part. empleados |
| 46 | `centro_costos` | `\|\|` | `\|\|` | `empleado_centro_costos` | Un centro de costos tiene su registro |
| 47 | `empleado_centro_costos` | `\|\|` | `\|{` | `empleado` | Un registro aplica a uno o muchos empleados |
| 48 | `empleado_centro_costos` | `\|\|` | `\|{` | `participante_empleado` | Un registro aplica a uno o muchos part. empleados |
...
|---------|-------|-------|-------------|
| 1.0.0 | 2026-03-27 | GitHub Copilot — Agente PO Método Ceiba | Creación inicial. Migración completa del archivo `docs/deprecated_docs/Diagrama entidad relacion.drawio` (draw.io 29.6.1, 5,059 líneas XML). Se extrajeron y documentaron 43 entidades con sus columnas, tipos de dato y claves, más 48 relaciones con notación crow's foot. Los diagramas fueron re-implementados en Mermaid `erDiagram`. Se organizó el contenido en 6 dominios funcionales para facilitar la comprensión del modelo. |
